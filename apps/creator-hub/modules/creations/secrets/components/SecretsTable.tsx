import {
  ...
  Button,
  TableSortLabel,
  TTableSortLabelProps,
  useDialog,
  AddIcon,
  Link,
} from '@rbx/ui';
import secretsClient, { Secret } from '@modules/clients/secrets';
...
import EditSecretDialog, { ApiError, formatError } from './EditSecret';
...
async function translateError(err: any): Promise<ApiError> {
  if ('response' in err) {
    ...
    try {
      ...
      let detail = '';
      if (body.title) detail = `${detail} ${body.title}`;
...
async function encryptSecret(
  ...
  await _sodium.ready;
  // eslint-disable-next-line camelcase -- These are library imports from libsodium-wrappers
  ...
  // eslint-disable-next-line camelcase -- Turned off to check in @rbx/ui upgrade
...
const SecretsTable: FunctionComponent<React.PropsWithChildren<SecretsTableProps>> = ({
  ...
  useEffect(() => {
    async function loadSecrets() {
      ...
      try {
        ...
        if (listed?.secrets) setSecrets(listed.secrets);
        setHasError(null);
      } catch (error) {
  ...
  return (
    <Grid container direction='column'>
      ...
      {
        // eslint-disable-next-line no-nested-ternary -- It is 3-way "if"
        hasError ? (
          // failed state
