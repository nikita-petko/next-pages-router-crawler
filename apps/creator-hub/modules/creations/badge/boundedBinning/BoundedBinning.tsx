...
function binContainsId(targetBin: OrderingBin, id: IdType): boolean {
  ...
  return upperBound === id || lowerBound === id || elements.includes(id as number);
}
...
class Binning implements BoundedBinning {
  ...
  private hasNotMoved(id: IdType): boolean {
    return !this.Delta.some((bin) => bin.elements.includes(id as number));
  }
  ...
  private isBinNecessary(targetBin: OrderingBin): boolean {
    ...
    for (let idIndex = 0; idIndex < targetBin.elements.length; idIndex += 1) {
      const id = targetBin.elements[idIndex];
  ...
  private trySplitBin(bin: OrderingBin): void {
    ...
    for (let i = 0; i < sourceElements.length; i += 1) {
      ...
      if (originalPositionOfElement === undefined) {
        throw Error('Attempted to access an element that is not defined.');
      }
      ...
      if (
        ...
        if (i === 0 && binToUpdate.upperBound) {
          ...
          for (
            let indexOfIdToCheck = 0;
            indexOfIdToCheck < sourceElements.length;
            indexOfIdToCheck += 1
          ) {
            const checkId = sourceElements[indexOfIdToCheck];
      ...
      for (let i = currentIndex - 1; i >= -1; i -= 1) {
        ...
        if (candidateId) {
          ...
          if (binCheckCandidates && binCheckCandidates.length > 0) {
            let overrodeNextBoundId: boolean = false;
            for (
              let binNumberToCheck = 0;
              binNumberToCheck < binCheckCandidates.length;
              binNumberToCheck += 1
            ) {
              const binToCheck = binCheckCandidates[binNumberToCheck];
        ...
        if (nextBoundId) {
          ...
          if (this.hasNotMoved(nextBoundId)) {
            ...
            for (
              let binNumberToMerge = 0;
              binNumberToMerge < binsToMergeCandidates.length;
              binNumberToMerge += 1
            ) {
              const binToMerge = binsToMergeCandidates[binNumberToMerge];
              const lowerBoundValue =
  ...
  private tryDrop(id: IdType, position: number): void {
    ...
    if (nextBin) {
      ...
      nextBin.elements.splice(currentPositionalDifference, 0, id as number);
