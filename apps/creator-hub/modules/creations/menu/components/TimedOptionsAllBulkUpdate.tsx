...
import type { RobloxItemConfigurationApiModelsRequestCollectiblesBulkUpdateTargetsAssetTypeEnum } from '@rbx/client-itemconfiguration/v1';
...
import { assetFullNameKeys, toastDurationTime } from '@modules/miscellaneous/common';
...
const TimedOptionsAllBulkUpdate: FunctionComponent<TimedOptionsAllBulkUpdateProps> = ({
  ...
  const getParentCheckState = (categoryKey: CategoryKey, assets: Asset[]) => {
    ...
    const isChecked = (a: Asset) => assetSelection[a];
  ...
  const handleConfirmationConfirm = useCallback(async () => {
    ...
    const selectedAssets = categoryOrder.flatMap((key) => {
      ...
      return displayAssetsByCategory[key].filter((asset) => assetSelection[asset]);
    });
  ...
  const confirmationCategorySummaryLines = useMemo(() => {
    const summaryLineForCategory = (key: CategoryKey): string | null => {
      ...
      const selected = displayed.filter((a) => assetSelection[a]);
  ...
  const hasAnyCheckboxSelected = useMemo(
    () =>
      categoryOrder.some(
        (key) => showCategory[key] && displayAssetsByCategory[key].some((a) => assetSelection[a]),
      ),
          ...
          <div id={panelId} className={classes.subtypeIndent}>
            {assets.map((asset) => (
              <Checkbox
                ...
                size='Small'
                placement='Start'
                isChecked={assetSelection[asset]}
