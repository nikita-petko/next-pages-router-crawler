...
import { Asset, components } from '@modules/miscellaneous/common';
...
import { dynamicAvatarItemsAssetTypes, getAllowedAssetTypes } from '../constants/MenuConstants';
...
const CreationsSubmenu: FunctionComponent<React.PropsWithChildren<TCreationsSubmenuProps>> = ({
  ...
  const [scrollLeft, setScrollLeft] = useState<number>(subMenuRef.current?.scrollLeft ?? 0);
  const [scrollWidth, setScrollWidth] = useState<number>(subMenuRef.current?.scrollWidth ?? 0);
  const [offsetWidth, setOffsetWidth] = useState<number>(subMenuRef.current?.offsetWidth ?? 0);
    ...
    return menuState.menuItem.submenuItems?.filter((submenuItem) =>
      creationsMenuManager.isMenuItemEnabled(
        ...
        menuState.menuItem.nameKey === 'Label.AvatarItems'
          ? dynamicAvatarItemsAssetTypes.has(submenuItem.type)
          : undefined,
        allowedAssetTypes,
      ),
    );
