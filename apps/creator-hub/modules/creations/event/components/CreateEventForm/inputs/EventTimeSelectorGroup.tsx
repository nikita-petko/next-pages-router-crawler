...
import type { Control, FormState, UseFormGetValues, UseFormTrigger } from 'react-hook-form';
...
> = ({ formState, control, getValues, trigger }) => {
  ...
  const getMinEndTime = useCallback(() => {
    ...
    if (invalidStartTime || Date.now() > startTime.getTime() + minimumEventDurationMs) {
