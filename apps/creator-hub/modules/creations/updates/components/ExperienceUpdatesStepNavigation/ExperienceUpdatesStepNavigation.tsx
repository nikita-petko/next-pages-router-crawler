...
import { toastDurationTime, FormMode, utils } from '@modules/miscellaneous/common';
import gameUpdateNotificationsClient, {
  GameUpdateMessageModel,
} from '@modules/clients/gameUpdateNotifications';
import GameUpdateNotificationsErrorCodesV2 from '@modules/creations/enums/GameUpdateNotificationsErrorCodes';
import { getResponseFromError } from '@modules/clients/utils';
import useExperienceUpdatesStepNavigationStyles from './ExperienceUpdatesStepNavigation.styles';
import { ExperienceUpdatesFormType } from '../types';
...
> = ({ experienceName, experienceId, experienceUpdatesList, onExperienceUpdatesListChange }) => {
  ...
  const timeIntervalTimestampValue = useMemo(() => {
    return utils.dayToMs(TimeInterval);
  }, []);

  const parseErrorMsg = useCallback(
    (errCode: number) => {
      let errorMsgKey = 'Error.UnknownError';
      const nameOfError = utils.getEnumKeyByValue(GameUpdateNotificationsErrorCodesV2, errCode);
      if (nameOfError) {
