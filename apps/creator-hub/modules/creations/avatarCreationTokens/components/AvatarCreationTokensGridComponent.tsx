...
> = ({ tokens, setItemType, itemType, setIsAssetType }) => {
  ...
  const onItemTypeChange = useCallback(
    (event: React.ChangeEvent<{ value: string }>) => {
      ...
      // oxlint-disable-next-line typescript/no-unsafe-type-assertion -- assertion only for Object.values membership check; result is the source of truth
      const isAssetType = Object.values(Asset).includes(parsedType as Asset);
      // oxlint-disable-next-line typescript/no-unsafe-type-assertion -- guarded by `isAssetType` and `Number(...)` coercion
...
export default withTranslation(AvatarCreationTokensGridComponent, [
  ...
  // `ConfigureItem` is loaded AFTER `AssetTypes` so that, per the
  // `Object.assign(...)` merge in `withTranslation`, its values win for any
  // shared keys. This makes the IEC item-type dropdown resolve
  // `Label.EyebrowAccessory` etc. to the verbose form ("Eyebrow Accessory")
  // used by the Configure form, while `AssetTypes` still provides labels for
  // non-IEC asset/bundle types (e.g. `Label.Hat`, `Label.Body`).
