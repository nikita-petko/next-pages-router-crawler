...
export type TDisplayInformationComponentProps = {
  onChange: (inputs: TAvatarCreationTokenDisplayInformation) => void;
  value: TAvatarCreationTokenDisplayInformation;
...
> = ({
  ...
  const handleValueChange = (newValues: TAvatarCreationTokenDisplayInformation) => {
    onChange(newValues);
  };
  ...
  const handleEnabledItemTypeChange = (e: React.ChangeEvent<{ value: string }>) => {
    // oxlint-disable-next-line typescript/no-unsafe-type-assertion -- Select options are sourced from enabledItemTypes; MUI emits an untyped value.
