...
const TryAssetForm: FunctionComponent<TryAssetFormProps> = ({
  ...
  const previewUrl = useMemo(() => {
    if (tryAssetMode === TryAssetMode.Disabled) return null;
  ...
  const [triggerValidationDebounced] = useDebouncedFunction(() => {
    trigger(TRY_ASSET_PLACE_ID_KEY);
  ...
  const onTryAssetModeChange = useCallback(
    (newMode: string, fieldOnChange: (value: string) => void) => {
      ...
      if (currentMode === TryAssetMode.Custom && newMode !== TryAssetMode.Custom) {
      ...
      } else if (newMode === TryAssetMode.Custom && savedPlaceIdRef.current) {
            ...
            <Controller
              ...
              render={({ field, fieldState }) => {
                const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
                  ...
                  const sanitizedInput = e.target.value.replace(/[^0-9]/g, '').slice(0, 19);
