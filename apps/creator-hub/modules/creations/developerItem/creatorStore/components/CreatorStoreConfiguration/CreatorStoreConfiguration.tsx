...
import { ModerationState, AssetType } from '@rbx/clients/assetsUploadApi';
import {
  assetToProduct,
...
import { FormProvider, SubmitHandler, useForm, FormState } from 'react-hook-form';
import {
  ThumbnailImageUploader,
  ThumbnailImageUploaderRef,
...
import useSocialLinks, {
  FetchSocialLinksResponse,
  SocialLinkWithType,
} from '../../hooks/useSocialLinks';
import usePreviews, { FetchPreviewIdsResponse } from '../../hooks/usePreviews';
...
export type CreatorStoreConfigurationType = BasicInfoDefaultFormType &
  DefaultSettingsFormType & {
    imagePreviewIds: number[];
    videoPreviewId: number | null;
    socialLinks: SocialLinkWithType[];
    tryAsset: TryAssetFormValue;
...
> = ({
  ...
  const refreshThumbnail = useCallback(() => {
    uploaderRef.current?.refreshThumbnail();
  }, []);
  ...
  // The use of this ref is to prevent the social links form from rendering.
  const areSocialLinksEnabledForUser = useRef(false); // Presume false until we know otherwise
    ...
    async function fetchData() {
      ...
      await Promise.all(requests).then(
        (responses) => {
          ...
          areSocialLinksEnabledForUser.current = responses[2] as boolean;
          if (previewsEnabledForUser) {
  ...
  const isItemDistributedValue = methods.watch('isItemDistributed');
  ...
  const configureDataSharing = useCallback(
    async (data: CreatorStoreConfigurationType) => {
      if (
        ...
        try {
          const dataLicenseList =
            data.dataSharingEnabled === true ? [DataSharingLicenseType.RobloxGlobal] : [];
          ...
          return Promise.reject(new Error(translate('Error.DataSharingConfigurationError')));
        }
      }
  ...
  const createMergedErrorMessage = useCallback(
    (results: PromiseSettledResult<void>[]) => {
      const rejectedResults = results.filter(
        (result) => result.status !== AllSettlePromiseSuccess && result.reason?.message,
      ) as PromiseRejectedResult[];
      const errorMessages = rejectedResults.map((result) => result.reason.message).join(', ');
  ...
  const onClickSave = useCallback(() => {
    ...
    const isModelBeingDistributedAndPriced =
      isItemDistributedValue &&
      JSON.parse(methods.getValues('fiatPrice'))?.quantity?.significand > 0;
    if (
      isEligibleForPaidModelConfirmationWarning &&
      ...
      methods.handleSubmit(validateAndSubmit)();
    }
  }, [
      ...
      <Grid container item classes={{ root: pageContainer }}>
        {isConfiguringThumbnailEnabled && (
          <Grid container item XSmall={12} classes={{ root: imageUploaderContainer }}>
            <ThumbnailImageUploader
              ...
              infoSection1={`${translate('Label.Format')} ${ThumbnailImageTypes.map(
                (type) => `*.${type}`,
              ).join(', ')}`}
        ...
        <Grid container item classes={{ root: formContainer }} alignItems='flex-start'>
          <Grid container item XSmall={12} Large={8} XLarge={6}>
            ...
            {areSocialLinksEnabledForUser.current && (
              <Grid item XSmall={12}>
