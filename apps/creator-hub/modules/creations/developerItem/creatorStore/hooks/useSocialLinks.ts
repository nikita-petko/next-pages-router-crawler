...
import { Asset, SocialLink } from '@rbx/clients/assetsUploadApi';
...
import { ApiPermissionStatus } from '@rbx/clients/assetPermissionsApi';
...
const getSocialLinksWithType = (asset: Asset): InternalSocialLinkWithType[] => {
  return Object.values(InternalSocialLinkType)
    ...
    .filter(Boolean) as InternalSocialLinkWithType[];
};
  ...
  socialLinksWithType.forEach((linkWithType) => {
    ...
    switch (linkWithType.type) {
      ...
      case InternalSocialLinkType.TRY_ASSET:
        ...
        break;
      default:
        throw new Error(`Unknown social link type: ${linkWithType.type}`);
    }
  });
...
const isPlacePublic = async (placeUniverseId: number) => {
 ...
 */
const getExistingTryAssetPlaceIsPlayable = async (placeId: number) => {
  try {
    ...
    const placeIsPublic = await isPlacePublic(placeUniverseId);
  ...
  const fetchSocialLinks = useCallback(async () => {
    ...
    const tryAssetExistingPlaceIsPlayable =
      tryAssetPlaceId && tryAssetPlaceId !== TRY_ASSET_DEFAULT_PLACE_ID
        ? await getExistingTryAssetPlaceIsPlayable(parseInt(tryAssetPlaceId, 10))
        : null;
      ...
      if (TRY_ASSET_ENABLED_ASSET_TYPES.includes(assetType)) {
        ...
        if (updatedTryAssetPlaceId) {
          internalUpdatedSocialLinksList.push({
            ...
            uri: updatedTryAssetPlaceId.toString(),
          });
        }
      }
  ...
  const validateTryAssetPlaceId = useCallback(
    async (placeId: string) => {
      try {
        ...
        const universeIsPublic = universeDetails?.privacyType === PUBLIC_UNIVERSE_PRIVACY_TYPE;
        if (!universeIsPublic) {
        ...
        const universeAssetPermission = await getUniverseHasPermission(Number(assetId), [
          universeId,
        ]);
