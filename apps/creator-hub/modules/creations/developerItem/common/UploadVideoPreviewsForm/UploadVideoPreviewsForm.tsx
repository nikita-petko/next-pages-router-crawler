...
export type UploadVideoPreviewsFormProps = {
  ...
  videoRejectionMessageOverride?: React.ReactNode;
  videoType: AssetType;
...
> = ({
  ...
  useEffect(() => {
    void fetchVideoQuota(videoType);
  ...
  const processVideoFile = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        await fetchVideoQuota(videoType);
    ...
    return (
      <div>
        ...
        {moderationState === ModerationState.Rejected && (
          <div className={rejectedMessagesContainer}>
            {videoRejectionMessageOverride ?? (
  ...
  const videoPreviewContent =
    videoModerationState === ModerationState.Approved ? videoPlayerComponent : moderationStatusBox;
            ...
            <>
              ...
              <Grid container item XSmall={12} spacing={2} className={mainContentContainer}>
                ...
                <Grid item XSmall={12} Large={8}>
                  {videoPreviewContent}
                  {videoPreviewId && <VideoDetailsFooter assetId={videoPreviewId} />}
                </Grid>
