...
import { BundleType } from '../constants/avatarItemConstants';
...
export interface CreationsDropdownCategorySubmenuProps {
  ...
  allowedBundleTypes?: Set<BundleType>;
}
...
> = ({
  ...
  function getDropdownKeys() {
    ...
    const filterBundles = (allowedBundleTypes?.size ?? 0) > 0;
    const filterAssets = (allowedAssetTypes?.size ?? 0) > 0;
    dropdownOptions.forEach((submenuItem) => {
      if (
        submenuItem.bundleType === BundleType.AvatarAnimations &&
        !settings.enableAvatarAnimations
      ) {
        return;
      }
      if (
        filterBundles &&
        submenuItem.itemType === Item.Bundle &&
        submenuItem.bundleType !== undefined &&
        !allowedBundleTypes?.has(submenuItem.bundleType)
      ) {
        return;
      }
      if (
        filterAssets &&
        submenuItem.assetType !== undefined &&
        !allowedAssetTypes?.has(submenuItem.assetType)
      ) {
        return;
      }
      const key =
