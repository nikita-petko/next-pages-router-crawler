...
import { dynamicAvatarItemsAssetTypes, getAllowedAssetTypes } from '../../menu';
...
const ItemCardContainer: FunctionComponent<React.PropsWithChildren<ItemCardContainerProps>> = ({
  ...
  useEffect(() => {
    if (dynamicAvatarItemsAssetTypes.size === 0) {
      getAllowedAssetTypes().then((allowedAssetTypes) => {
        setIsMarketplaceAssetType(allowedAssetTypes.has(item.assetType as Asset));
      });
    } else {
