...
import { PagingParameters, PageResponse } from '@rbx/core';
...
function ItemGridContainer<T, P extends PagingParameters>({
  ...
  const onDataLoad = useCallback(
    (data: T[], isFirstPage: boolean) => {
      ...
      fetchItemDetailsAndUpdate(data);
    },
  ...
  useEffect(() => {
    if (updateItems) {
      setPageItems(updateItems(currentPageItems || (emptyItems as T[])));
    } else {
      setPageItems(currentPageItems || (emptyItems as T[]));
    }
