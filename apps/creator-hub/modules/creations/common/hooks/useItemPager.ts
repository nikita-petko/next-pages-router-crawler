...
function useItemPager<T, P extends PagingParameters>(
  ...
  const removeItemAtIndex = useCallback(
    async (index: number) => {
      ...
      if (!isLastItemInPage) {
        ...
        void cursorPager.removeItemAtIndex(index);
  ...
  const reloadCurrentPage = useCallback(() => {
    void withLoadingState(cursorPager.reloadCurrentPage());
  }, [cursorPager, withLoadingState]);
  ...
  const updateItemAtIndex = useCallback(
    (index: number, itemData: T) => {
      ...
      void withLoadingState(cursorPager.updateItemAtIndex(index, itemData));
    },
    [cursorPager, withLoadingState],
  );
