import { assetPermissionsApiClient, developClient } from '@modules/clients';
...
import { RobloxApiDevelopModelsResponsePlaceMediaItemResponse } from '@rbx/clients/develop';
        ...
        validMediaItems.map((item, index) => {
          ...
          switch (type) {
            ...
            case MediaType.Video:
              return {
                id: item.videoAssetId as number,
                type,
    ...
    queryFn: async () => {
      ...
      const assetPermissionChecks = await assetPermissionsApiClient.batchCheckAssetPermissions(
        validMediaItems.map((item) => {
          return {
            assetId: isYoutubeVideo(item)
              ? (item.videoAssetId as number)
