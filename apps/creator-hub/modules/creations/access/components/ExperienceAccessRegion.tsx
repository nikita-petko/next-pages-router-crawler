...
function ExperienceAccessRegion({ methods: { control }, allCountries }: Props) {
  const {
    classes: {
      autocomplete,
      section,
      ...
      <Grid item XSmall={12} classes={{ root: autocomplete }}>
        <Controller
          ...
          render={({ field }) => {
            ...
            const displayValue = isAllSelected ? [allRegionsOption, ...currentValue] : currentValue;
              ...
              <Autocomplete
                ...
                onChange={(event, newValue) => {
                  ...
                  if (newAllSelected !== isAllSelected) {
                    ...
                    const allowedCountries = newValue.filter((item) => item.countryCode !== 'ALL');
