...
import { developClient, universesClient } from '@modules/clients';
...
const PlaceAccessForm: FunctionComponent<React.PropsWithChildren<PlaceAccessFormProps>> = ({
  placeId,
  maxPlayersAllowed,
  maxPlayerCount,
  socialSlotStrategy,
...
responsible for triaging issue. */
  ...
  const getElementWithInfo = (node: React.ReactNode, information: string) => {
    return (
        ...
        <Grid className={maxPlayer} item XSmall={12} Large={3}>
          <Controller
            ...
            rules={{
              ...
              validate: {
                pattern: () => {
                  /* eslint-disable-next-line prefer-regex-literals -- NOTE
(jcountryman, 2/6/24): Turned off to check in @rbx/ui upgrade. Codeowners is
responsible for triaging issue. */
