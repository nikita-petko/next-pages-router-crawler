...
const parseTimeSpan = (timeSpan: string): number => {
  const [seconds, minutes, hours, days] = timeSpan.split(/[:.]/).reverse().map(Number);
...
function areCachedDatesValid(
  ...
  if (!startValid && cachedStart !== null) return null;
  ...
  if (cachedEnd !== null && !endValid) return null;
...
function ScheduleReleaseDialog({
  ...
  useEffect(() => {
    ...
    if (!cached || (cached.startDate === null && cached.endDate === null)) return;
