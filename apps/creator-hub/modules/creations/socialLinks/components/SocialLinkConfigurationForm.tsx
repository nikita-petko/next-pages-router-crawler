...
import type { SocialLinksData, SocialLinkTypes } from '@modules/clients/games';
...
> = ({ linkMetadata, onAdd, onSave, onDelete, onRefetchLinks, savedLinks }) => {
  ...
  const triggerInputValidation = useCallback(async () => {
    await trigger(fields.map((_, index) => `socialLink.${index}.linkType` as const));
    ...
    fields.forEach((_, index) => {
      if (
        ...
        getValues(`socialLink.${index}.linkType` as const) === null
      ) {
        clearErrors(`socialLink.${index}.linkType` as const);
      }
    });
