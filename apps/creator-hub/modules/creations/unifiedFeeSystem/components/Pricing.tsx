...
import { DefaultMaxCollectiblePrice } from '../helper/UnifiedFeeSystemConstants';
...
function Pricing(props: PricingProps) {
  ...
  const showItemPrice = is2dAsset || !isFree;
  const showRegionalPricing =
    is2dAsset || (!isLimited && collectiblesMetadata?.isRegionalPricingEnabled);
  const showTimedOptions = !is2dAsset && isRentablesEnabled && isRentableOptIn;
  ...
  return (
    <div>
      <div style={{ opacity: showItemPrice ? '100%' : '24%' }}>
        ...
        <Grid container item XSmall={12} rowGap={2} style={{ marginTop: '40px', color: 'inherit' }}>
