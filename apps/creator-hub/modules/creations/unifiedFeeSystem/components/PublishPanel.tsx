...
import { Asset, Item as ItemType } from '@modules/miscellaneous/common';
...
const formatPublishingFees = (fees: number | undefined) => {
  return fees ? fees.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',') : '-';
};
  ...
  useEffect(() => {
    async function fetchAllowedPublishTypes() {
      try {
        ...
        const allowedTypes = [
          ...(response.allowedAssetTypes?.map(normalizeType) ?? []),
          ...(response.allowedBundleTypes?.map(normalizeType) ?? []),
        ];
        setIsPublishAllowedForItemType(allowedTypes.includes(itemType.toLowerCase()));
      } catch {
        setIsPublishAllowedForItemType(true);
      }
    }
    fetchAllowedPublishTypes();
  }, [itemType]);
