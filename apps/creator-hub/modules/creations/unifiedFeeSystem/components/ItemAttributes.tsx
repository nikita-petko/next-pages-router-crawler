          ...
          {displayNonLimited ? (
            ...
            <Grid item XSmall={12} Large={7}>
              <ToggleButtonGroup
                value={isLimited}
                exclusive
                onChange={(event, newIsLimited: boolean) => {
            ...
            <Grid item XSmall={12} Large={7}>
              <FormControl component='fieldset'>
                <RadioGroup
                  ...
                  name='wearTime'
                  value={wearTime}
                  // oxlint-disable-next-line @typescript-eslint/no-unsafe-type-assertion -- ToggleButtonGroup value is typed as string
                  ...
                  {DurationOptions.map((option) => {
                    ...
                    if (isUsed) {
                      return (
                        <Tooltip
                          key={option}
                          title='Lorem ipsum' // oxlint-disable-line rbx/no-hardcoded-translation-string -- TODO @mryumae
                          arrow>
                          <FormControlLabel
                            value={option}
        ...
        {isLimited ? (
          <>
            <Grid
              ...
              className={!isLimited ? classes.nonLimited : classes.limited}>
              <Grid item XSmall={12} Large={5}>
                {/* oxlint-disable-next-line rbx/no-hardcoded-translation-string -- pre-existing; should use translate('Label.Quantity') */}
            ...
            <Grid
              ...
              <Grid item XSmall={12} Large={7}>
                <TextField
                  ...
                  disabled={!isLimited}
                  error={(limit ?? 0) > (quantity ?? 0) || (limit ?? 0) > 500}
                  value={isLimited && (limit ?? 0) > 0 ? limit : ''}
                  helperText={
                    (limit ?? 0) > (quantity ?? 0) || (limit ?? 0) > 500
                      ? translate('Label.LimitHelperQuantity')
                      : ''
                  }
                  onChange={(event) => {
                    const value = +event.target.value;
              ...
              <Grid item XSmall={12} Large={5}>
                ...
                {isBundle && (
                  <>
                    <br />
                    <Typography variant='body2' className={classes.description}>
                      {translate('Message.BundleCannotBeFree')}
                    </Typography>
                  </>
                )}
              </Grid>
