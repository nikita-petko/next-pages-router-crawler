...
import getBundleTypeToBundleTypeString from './unifiedFeeSystemBundleMapping';
...
export function mapSaleLocationToType(
  ...
  // eslint-disable-next-line typescript-eslint/switch-exhaustiveness-check -- intentional default fallback for unmapped enum members
  switch (saleLocation) {
...
export function mapAssetTypeToString(
  ...
  switch (assetType) {
    ...
    case RobloxItemConfigurationApiAssetDetailsAssetTypeEnum.NUMBER_92:
      return Asset.AvatarBackground.toString();
    default:
      return 'Invalid';
  }
}
...
export async function getValidWearTimeTypes() {
  ...
  if (response.allowedAssetTypes) {
    response.allowedAssetTypes.forEach((assetType) => {
      // eslint-disable-next-line typescript/no-unsafe-type-assertion -- API returns asset-type strings that map to the Asset enum
