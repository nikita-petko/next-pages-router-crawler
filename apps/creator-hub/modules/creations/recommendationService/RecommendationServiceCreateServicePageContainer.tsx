...
type BaseTemplate = 'maximize_engagement' | 'recently_added' | 'player_specific';
type StoredBaseTemplate = 'MaximizeEngagement' | 'RecentlyAdded' | 'PlayerSpecific';
...
const daysToLookbackWindow = (days: unknown): LookbackWindow | null => {
  ...
  if (asNumber === 7) return '7d';
  if (asNumber === 28) return '28d';
  if (asNumber === 90) return '90d';
  return null;
};
...
const coerceContentTypeFilterValues = (value: unknown): ContentTypeFilterValue[] | null => {
  ...
  const mapped = value
    .map((v): ContentTypeFilterValue | null => {
      ...
      if (normalized === 'static') return 1;
      if (normalized === 'dynamic') return 2;
      if (normalized === 'interactive') return 3;
      return null;
    })
...
export function RecommendationServiceServiceWizard({
  ...
  const includeExploration = baseTemplate === 'maximize_engagement';
  const includeRanking = baseTemplate === 'maximize_engagement';

  useEffect(() => {
    if (baseTemplate === 'maximize_engagement') return;
  ...
  const stepperStepOrder = useMemo((): WizardStep[] => {
    ...
    if (includeExploration) steps.push(2);
    if (includeRanking) steps.push(3);
    steps.push(4, 5);
    return steps;
  ...
  const hasInvalidExplorationSlotsInput = useMemo(() => {
    if (!isExplorationEnabled) return false;
    if (explorationSlotsTokens.length === 0) return false;
  ...
  const canAdvanceFromStep = useMemo(() => {
    ...
    const isLookbackWindowRequired = baseTemplate === 'maximize_engagement';

    if (step === 0) return isConfigKeyValid && baseTemplate != null;
    if (step === 1)
  ...
  useEffect(() => {
    ...
    const storedValue = (entries as Record<string, unknown>)[editKey];
    ...
    if (
      ...
      if (asWindow) setLookbackWindow(asWindow);
    }
  ...
  const configValue = useMemo(() => {
    const retrievalWindowInDays =
      baseTemplate === 'maximize_engagement' && lookbackWindow
    ...
    return {
      ...
      ...(includeRanking
        ...
        : {}),
      ...(baseTemplate === 'maximize_engagement'
        ? {
  ...
  const persistMutation = useMutation({
    mutationFn: async () => {
      ...
      if (mode === 'create') {
        if (existingEntries && Object.prototype.hasOwnProperty.call(existingEntries, key)) {
  ...
  const onNext = useCallback(async () => {
    ...
    setStep((prev) => {
      ...
      if (prev === 1) {
        if (includeExploration) return 2;
        if (includeRanking) return 3;
        return 4;
      }
      if (prev === 2) {
        if (includeRanking) return 3;
  ...
  const content = useMemo(() => {
    if (step === 0) {
      ...
      const configKeyErrorMessage = (() => {
        if (shouldShowConfigKeyRequiredError) return configKeyRequiredError;
        if (hasInvalidConfigKeyChars) return configKeyAllowedCharsError;
        return undefined;
      })();
    ...
    if (step === 1) {
      ...
      const lookbackWindowSection = (() => {
        if (baseTemplate !== 'maximize_engagement') return null;
        ...
        <div className='flex flex-col gap-medium padding-xlarge'>
          ...
          <div className='flex items-center gap-small'>
            <Button
              variant='Standard'
              size='Large'
              onClick={() => refetchExistingValues().catch(() => undefined)}>
              {tryAgainLabel}
            </Button>
            <Button variant='Standard' size='Large' onClick={onCancel}>
              {cancelLabel}
...
const RecommendationServiceCreateServicePageContainer: FC = () => {
  return (
    <AnalyticsFlagGatedContext flag='recommendationServicesConfigEnabled'>
      <RecommendationServiceServiceWizard mode='create' />
    </AnalyticsFlagGatedContext>
  );
};
