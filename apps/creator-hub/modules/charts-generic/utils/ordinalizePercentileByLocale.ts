...
const ordinalizePercentileByLocale = (n: number, locale: Locale) => {
  ...
  switch (locale) {
    case Locale.English: {
      if (rule === 'one') return `${n}st`;
      if (rule === 'two') return `${n}nd`;
      if (rule === 'few') return `${n}rd`;
      return `${n}th`;
    }
    case Locale.French: {
      if (rule === 'one') return `${n}re`;
      return `${n}e`;
    }
    case Locale.Italian:
      return `${n}.ᵊ`;
    ...
    case Locale.Vietnamese:
      throw new Error(`Ordinal number format not implemented yet for locale ${locale}`);
    default: {
      const exhaustiveCheck: never = locale;
      throw new Error(`Unknown ordinal number format for locale ${exhaustiveCheck}`);
    }
  }
};
