// eslint-disable-next-line eslint-comments/disable-enable-pair -- for obvious reasons
/* eslint-disable max-classes-per-file -- errors are considered classes apparently */
import {
  RAQIBreakdownValue,
  RAQIDatapoint,
  RAQIMetricValue,
...
const validateBreakdown = <TDimension extends string>(
  ...
  if (!value) throw new RAQIValidationBreakdownValueNotFoundError();
  if (!dimension) throw new RAQIValidationBreakdownDimensionNotFoundError();
...
const validateDatapoint = (input: RAQIUnvalidatedDatapoint): RAQIDatapoint => {
  ...
  if (!timestamp) throw new RAQIValidationDatapointTimestampNotFoundError();
...
const validateMetricValue = <TDimension extends string>(
  ...
  if (!datapoints) throw new RAQIValidationSeriesDatapointsNotFoundError();
  return {
...
export const validateResponse = <TDimension extends string>(
  ...
  const { values } = input;
  if (!values) throw new RAQIValidationResponseValuesNotFoundError();
