...
import { Status, TDotStatus, TIconStatus, TStatus } from './types/GenericTableType';
...
const IconStatusChip: FC<IconStatusChipProps> = ({ status }) => {
  ...
  const icon = useMemo(() => {
    switch (preset) {
      ...
      default: {
        const exaustiveCheck: never = preset;
        throw new Error(`Unhandled preset: ${exaustiveCheck}`);
      }
    }
  }, [preset]);
...
const StatusChip: FC<StatusChipProps> = ({ status }) => {
  ...
  switch (chipType) {
    ...
    default: {
      ...
      throw new Error(`Unhandled status chipType: ${exhaustiveCheck}`);
    }
  }
};
