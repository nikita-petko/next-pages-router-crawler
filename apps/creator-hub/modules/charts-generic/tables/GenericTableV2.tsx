import React, { ReactElement, ReactNode, useCallback, useMemo, useState, useEffect } from 'react';
...
const SkipRowExpansionToggleSelector =
  'button, a, input, select, textarea, [role="button"], [role="link"], [data-disable-row-expansion-toggle="true"]';
...
>({
  ...
  // Sync header state when parent updates defaultActiveSort
  useEffect(() => {
    const initial = columnConfigs.find((c) => c.columnKey === tableConfig?.defaultActiveSort);
    setOrder(initial?.sort?.direction ?? undefined);
    setOrderBy(tableConfig?.defaultActiveSort);
  }, [tableConfig?.defaultActiveSort, columnConfigs]);
  ...
  const expandedRowKeys =
    rowExpansion?.expandedRowKeys !== undefined
  ...
  const renderedRows = useMemo(() => {
    return effectiveParentRows.flatMap(({ rowInfo, rowIndex, rowKey, isRowSelected }) => {
      ...
      const parentRow = (
        <GenericTableRow
          ...
          {unHiddenColumnConfigs.map((config, colIndex) => {
            ...
            const showToggleInFirstCell = rowExpansion?.showToggleInFirstCell ?? true;
            const toggleContent =
              colIndex === 0 && isRowExpandable && showToggleInFirstCell ? (
                <IconButton
                  ...
                  {isRowExpanded ? <ExpandMoreIcon /> : <ChevronRightIcon />}
                </IconButton>
              ) : null;
            const contentWithOptionalToggle = toggleContent ? (
              <Flex alignItems='center' gap={4}>
                <span
                  data-disable-row-expansion-toggle='true'
                  style={{ display: 'inline-flex', alignItems: 'center' }}>
                  {toggleContent}
                </span>
                <div style={{ flex: 1, minWidth: 0 }}>{cellContent}</div>
              </Flex>
            ) : (
              cellContent
            );
      ...
      const seenExpandedRowIdentities = new Map<string, number>();
      const expandedRows = Array.from({ length: expandedRowCount }).flatMap(
        (_, expandedRowIndex) => {
          const expandedRowIdentity = expandedColumnEntries
            .map((entry) => {
              const expandedCellSpec = entry.expandedCellSpecs[expandedRowIndex];
              if (expandedCellSpec?.skipCell) {
                return `${String(entry.expandedColumnConfig.columnKey)}:skip`;
              }
              const expandedCellValue = expandedCellSpec?.cellData;
              if (expandedCellValue === undefined || expandedCellValue === null) {
                return `${String(entry.expandedColumnConfig.columnKey)}:`;
              }
              return `${String(entry.expandedColumnConfig.columnKey)}:${JSON.stringify(expandedCellValue)}:${expandedCellSpec?.colSpan ?? 1}`;
            })
            .join('|');
          const expandedRowIdentityCount = seenExpandedRowIdentities.get(expandedRowIdentity) ?? 0;
          seenExpandedRowIdentities.set(expandedRowIdentity, expandedRowIdentityCount + 1);
          const expandedRowKey =
            expandedRowIdentityCount === 0
              ? `${rowKey}-expanded-${expandedRowIdentity}`
              : `${rowKey}-expanded-${expandedRowIdentity}-${expandedRowIdentityCount}`;
          ...
          return (
            <GenericTableRow
              key={expandedRowKey}
              selected={isRowSelected}
              style={{ backgroundColor: theme.palette.surface[100] }}>
              {expandedCells}
            </GenericTableRow>
          );
        },
