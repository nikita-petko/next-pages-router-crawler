...
import { ColumnType, ColumnTypeToAlign, resolveTableColumnTitle } from './types/GenericColumnType';
...
const GenericTableHeaderCell = <TColumnKey extends string | number>({
  ...
  const {
    classes: { headerCell, hiddenSortIcon, label, labelIcon },
  ...
  return (
    <GenericTableCell
      ...
      {!config.sort ? (
        ...
        <TableSortLabel
          component='div'
          classes={{ root: label, icon: config.sort.hideSortIcon ? hiddenSortIcon : labelIcon }}
          active={orderBy === config.columnKey}
          disabled={isDataLoading}
          direction={direction}
          hideSortIcon={config.sort.hideSortIcon}
          onClick={sortOnClick}>
          {cellContent}
        </TableSortLabel>
      )}
