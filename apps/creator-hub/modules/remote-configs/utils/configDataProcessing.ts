...
export const filterAndSortConfigEntries = (
  ...
  const getLastModifiedTime = (detail: ValidConfigEntryDetail) => {
    if (!detail.isOverride) return null;
    if (!detail.overrideEntry?.lastModifiedTime) return null;
  ...
  filtered.sort((a, b) => {
    ...
    switch (sortKey) {
      ...
      case SortKey.LastModifiedTime: {
        ...
        if (aModTime === null) return 1;
        if (bModTime === null) return -1;
      ...
      case SortKey.LastAccessedTime: {
        ...
        if (aAccessTime === null) return 1;
        if (bAccessTime === null) return -1;
...
export const paginateEntries = <T>(entries: T[], maxPageSize?: number, skip?: number): T[] => {
  if (!maxPageSize || maxPageSize <= 0) return entries;
  const start = skip || 0;
