...
export const makeEntryValue = (
  ...
  switch (overrideType) {
    ...
    default: {
      return assertNever(overrideType, 'Unexpected override type');
    }
  }
};
...
export const stringValueFromEntryValue = (entryValue: ValidConfigEntryValue): string => {
  switch (entryValue.valueType) {
    ...
    default: {
      return assertNever(entryValue, 'Unexpected value type');
    }
  }
};
...
export const toClauseTokens = (clause: TargetingClauseFormData): Array<ConditionRuleToken> => {
  if (!clause.dimension || clause.values.length === 0) {
    return [];
  }
...
const defaultDimension = RAQIV2Dimension.Country;
...
export const createDefaultTargetingState = (): Pick<
  TargetingConditionFormData,
  'mode' | 'conditionName' | 'clauses'
> => ({
...
export const createDefaultConditionValue = (): TargetingConditionFormData => ({
  id: makeTargetingId(),
  ...createDefaultTargetingState(),
  conditionalStringValue: '',
  conditionalBoolValue: 'true',
});

export const normalizeClauseJoiners = (
  clauses: ReadonlyArray<TargetingClauseFormData>,
  joinerToNext: TargetingClauseFormData['joinerToNext'],
