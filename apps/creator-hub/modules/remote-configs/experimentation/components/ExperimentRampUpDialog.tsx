...
import usePValueForExperimentMetrics from '../hooks/usePValueForExperimentMetrics';
...
const ExperimentRampUpDialog = ({
  ...
  const { pValueByExperimentMetricAndVariant } = usePValueForExperimentMetrics(experimentId);
  ...
  const alert: null | {
    ...
    if (!experiment || !isExperimentStoppable(experiment.state)) return null;
  ...
  }, [experiment, hideAlert, locale, pValueByExperimentMetricAndVariant, translate]);
  ...
  const alertAction = useMemo(() => {
    return (
      <Link href={urls.creatorHub.docs.getExperimentationBestPracticesUrl()} target='_blank'>
