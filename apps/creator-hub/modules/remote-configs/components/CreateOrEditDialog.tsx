...
import InputFieldWrapper from '@modules/charts-generic/components/InputFieldWrapper';
...
const CreateOrEditDialog = ({ open, edit, onClose }: CreateOrEditDialogProps) => {
      ...
      if (initialValue) {
        ...
        switch (valueType) {
          ...
          default: {
            ...
            throw new Error(`Unexpected value type: ${String(exhaustiveCheck)}`);
          }
        }
      }
    } else {
    ...
    (ev) => {
      ...
      switch (overrideType) {
        ...
        default: {
          ...
          throw new Error(`Unexpected override type: ${String(exhaustiveCheck)}`);
        }
      }
  ...
  const isDialogError = isKeyError || !configValueValidationResult.isValid || isDescriptionError;
      ...
      return (
        <Select
          ...
          <MenuItem value='true'>
            {tPendingTranslation(
              'True',
              'Option label for the boolean value true in the config value selector.',
              translationKey(
                'Label.ConfigCreation.Value.True',
                TranslationNamespace.UniverseConfigAndExperimentation,
              ),
            )}
          </MenuItem>
          <MenuItem value='false'>
            {tPendingTranslation(
              'False',
              'Option label for the boolean value false in the config value selector.',
              translationKey(
                'Label.ConfigCreation.Value.False',
                TranslationNamespace.UniverseConfigAndExperimentation,
              ),
            )}
          </MenuItem>
    ...
    return (
      <Select
        ...
        onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
          ...
          if (isValidEnumValue(ValidConfigEntryValueType, selectedValue)) {
            setOverrideType(selectedValue);
          }
        }}
        ...
        <MenuItem value={ValidConfigEntryValueType.String}>
          {tPendingTranslation(
            'String',
        ...
        <MenuItem value={ValidConfigEntryValueType.Number}>
          {tPendingTranslation(
            'Number',
        ...
        <MenuItem value={ValidConfigEntryValueType.Boolean}>
          {tPendingTranslation(
            'Boolean',
        ...
        <MenuItem value={ValidConfigEntryValueType.Json}>
          {tPendingTranslation(
            'JSON',
  ...
  }, [isEditing, fields, overrideType, translate, tPendingTranslation]);

  const dialogErrorMessage = useMemo(() => {
    const mutationError = isEditing ? updateError : createError;
    const translatedCommonErrorMessage =
      mutationError?.getTranslatedErrorMessage(tPendingTranslation);
    if (translatedCommonErrorMessage) {
      return (
        <Typography variant='h6' color='error' sx={{ marginRight: 1 }}>
          {translatedCommonErrorMessage}
        </Typography>
      );
    }
    return null;
  }, [createError, isEditing, tPendingTranslation, updateError]);
