...
type ConfigCreationDefineKeyStepProps = {
  ...
  onNext: () => void;
  onCancel: () => void;
  // When provided, action buttons are portaled into this container so the
  // parent can render them in a page-level sticky footer. Falls back to
  // inline rendering otherwise (tests, standalone usage).
...
const ConfigCreationDefineKeyStep: FC<ConfigCreationDefineKeyStepProps> = ({
  ...
  const valueErrorMessage = useMemo(() => {
    return !valueValidation.isValid && shouldShowValueError ? valueValidation.message : undefined;
  }, [shouldShowValueError, valueValidation]);
  const {
    isError: hasDescriptionError,
    charCount: descriptionCharCountText,
    helperText: descriptionHelperText,
  } = useConfigDescriptionField(description ?? '');
  const isFirstStepSubmitDisabled =
    isStepTransitionPending || hasKeyError || !valueValidation.isValid || hasDescriptionError;
  ...
  return (
    <div>
          ...
          <Controller
            ...
            render={({ field }) => (
              <Dropdown
                ...
                onValueChange={(value) => {
                  if (isValidEnumValue(ValidConfigEntryValueType, value)) {
      ...
      <div className='margin-bottom-large'>
        ...
        />
        <div
          className={`margin-top-xsmall text-body-small ${
            hasDescriptionError ? 'content-alert' : 'content-muted'
          }`}>
          {hasDescriptionError ? descriptionCharCountText : descriptionHelperText}
        </div>
      </div>
