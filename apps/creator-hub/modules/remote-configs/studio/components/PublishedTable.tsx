...
import type { ActionsForConfigEntry, ActionInvokers } from '../../hooks/useConfigEntriesActions';
import { RemoteConfigAction, useConfigEntriesActions } from '../../hooks/useConfigEntriesActions';
...
const orderedActionsWithStagedChangesAsMenuOptions = [
  RemoteConfigAction.CopyConfigSnippet,
...
const PublishedTableRow = ({
  ...
  const actions = useMemo(() => {
    ...
    const actionsList = config.isStaged
      ? orderedActionsWithStagedChangesAsMenuOptions
      : orderedActionsAsMenuOptions;
    return actionsList.map((actionType) => configEntriesActions[actionType]);
  ...
  return (
    <div className={strictly(tableRow, tableRowNonHeader)}>
      <div
        className={strictly(keyColumn, 'select-all')}
        style={config.isStaged ? { fontStyle: 'italic' } : undefined}>
        {config.isStaged ? `${key}*` : key}
      </div>
