import { DeploymentResult } from '@rbx/client-creator-configs-api/v100';
import { ConfigType } from '@rbx/client-universe-configs-web-api/v1';
...
import type { ValidExperimentConfigEntry } from './validExperimentationTypes';
import type {
...
type ApiSerializedConfigEntryValue =
  | string
  | number
  | boolean
...
const parseJsonConfigEntryValue = (jsonValue: string): Record<string, unknown> | unknown[] => {
  const parsed: unknown = JSON.parse(jsonValue);
  if (isRecord(parsed) || Array.isArray(parsed)) {
    return parsed;
  }
  throw new Error('JSON config entry value must be an object or array');
};
...
export const toValidConditionalEntryIdentity = (
  entry: DangerousConditionalConfigEntry,
): {
  key: string;
  conditionName: string;
...
export const toValidExperimentConfigEntry = (
  entry: DangerousValueConfigEntry,
): ValidExperimentConfigEntry => {
  if (!entry.key) {
    throw new Error('ConfigEntry key is required');
  }
  return {
    key: entry.key,
    entryValue: entry.entryValue == null ? null : toValidConfigEntryValue(entry.entryValue),
  };
};
...
const toValidChangelogConfigEntry = (entry: DangerousConfigEntry): ValidConfigEntry => {
  ...
  const entryValue = toValidConfigEntryValue(
    isDangerousRuleConfigEntry(entry)
      ? (entry.ruleData?.rule ?? entry.entryValue)
      : isDangerousRuleOrderingConfigEntry(entry)
        ? (entry.ruleOrderingData?.conditionOrder ?? entry.entryValue)
        : entry.entryValue,
  );
  let conditionValue: Map<string, ValidConfigEntryValue> | undefined;
  if (isDangerousConditionalConfigEntry(entry)) {
    const conditionName = entry.conditionalValueData?.conditionName;
    if (conditionName) {
      conditionValue = new Map([[conditionName, entryValue]]);
    }
  }
...
export const toValidConfigEntryStaged = (
  dangerousConfigEntryStaged: DangerousConfigEntryStaged,
): ValidConfigEntryStaged | null => {
  const { isPublishing: isPublishingGiven, isDeleted, overrideEntry } = dangerousConfigEntryStaged;
  const isPublishing = isPublishingGiven ?? false;
  const currentValue =
    dangerousConfigEntryStaged.currentValue != null
      ? toValidConfigEntryValue(dangerousConfigEntryStaged.currentValue)
      : null;
  ...
  if (isDeleted || overrideEntry.entry?.entryValue == null) {
    ...
    try {
      ...
      logAnalyticsError(
        `Error validating ConfigEntryStaged overrideEntry: ${err instanceof Error ? err.message : String(err)}`,
      );
      return null;
    }
  }
...
export const toValidConditionRule = (entry: DangerousRuleConfigEntry): ValidConditionRule => {
...
export const toValidRuleOrdering = (entry: DangerousRuleOrderingConfigEntry): ValidRuleOrdering => {
...
const toValidGetStagedChangesResponse = (
  ...
  dangerousEntries.forEach((entry) => {
    ...
    if (dangerousOverrideEntry && isDangerousConditionalConfigEntry(dangerousOverrideEntry)) {
      const { key, conditionName } = toValidConditionalEntryIdentity(dangerousOverrideEntry);
      const currentConditionValue =
        entry.currentValue != null ? toValidConfigEntryValue(entry.currentValue) : undefined;
      const isConditionDeleted =
        entry.isDeleted === true || dangerousOverrideEntry.entryValue == null;
      ...
      if (isConditionDeleted) {
        if (currentConditionValue) {
          if (existingEntryIndex === undefined) {
            const pendingCurrentConditionValues =
              pendingCurrentConditionValuesByKey.get(key) ??
              new Map<string, ValidConfigEntryValue>();
            pendingCurrentConditionValues.set(conditionName, currentConditionValue);
            pendingCurrentConditionValuesByKey.set(key, pendingCurrentConditionValues);
          } else {
            ...
            existingEntry.currentConditionValue ??= new Map();
            existingEntry.currentConditionValue.set(conditionName, currentConditionValue);
          }
        }
        ...
        if (existingEntryIndex !== undefined) {
          ...
          existingEntry.overrideEntry.entry.conditionValue?.delete(conditionName);
        }
    ...
    if (validatedEntry) {
      ...
      let existingCurrentConditionValues: Map<string, ValidConfigEntryValue> | undefined;
      if (existingEntryIndex !== undefined) {
        const existingEntry = entries[existingEntryIndex];
        if (!existingEntry.isDeleted) {
          existingConditionValues = existingEntry.overrideEntry.entry.conditionValue;
          existingCurrentConditionValues = existingEntry.currentConditionValue;
        }
      }
      const pendingConditionValues = pendingConditionValuesByKey.get(key);
      ...
      pendingDeletedConditionNames?.forEach((conditionName) => {
        validatedEntry.overrideEntry.entry.conditionValue?.delete(conditionName);
      });
      if (pendingDeletedConditionNames) {
        pendingDeletedConditionNamesByKey.delete(key);
      }
      if (pendingCurrentConditionValues) {
        validatedEntry.currentConditionValue = existingCurrentConditionValues
          ? new Map<string, ValidConfigEntryValue>([
              ...existingCurrentConditionValues,
              ...pendingCurrentConditionValues,
            ])
          : pendingCurrentConditionValues;
        pendingCurrentConditionValuesByKey.delete(key);
      } else if (existingCurrentConditionValues) {
        validatedEntry.currentConditionValue = existingCurrentConditionValues;
      }
  ...
  if (
    ...
    if (
      ...
      // A stale delete-only conditional delta has no meaningful parent row to render if the
      // latest config response no longer has its base entry. Drop just that orphaned delta
      // instead of failing the whole staged response.
...
const toValidChangelogEntry = (
  ...
  return {
    ...
    publishedBy:
      typeof dangerousChangelogEntry.publishedBy === 'number'
        ? dangerousChangelogEntry.publishedBy
        : Number(dangerousChangelogEntry.publishedBy),
...
export const toAPIConfigEntryValue = (
  ...
  switch (internalEntryValue.valueType) {
    ...
    case ValidConfigEntryValueType.Json:
      return parseJsonConfigEntryValue(internalEntryValue.jsonValue);
    default: {
...
const toApiDeleteConditionalValueData = (conditionName: string): ApiUpdateConditionalValueData => {
  return {
    conditionName,
    isDeleted: true,
  };
...
const toApiUpdateConfigurationDataV2 = (
  ...
  // Optional staged-edit behavior: for matching condition names, issue a
  // delete in update list and recreate in create list.
  conditionNamesToRecreate: Array<string> | undefined,
  internalConditionalRules: Array<ValidConditionRule>,
  ...
  const conditionNamesToDeleteSet = new Set(conditionNamesToDelete);
  if (!conditionValue?.size) {
    if (conditionNamesToDeleteSet.size > 0) {
      translatedUpdateData.toUpdateConditionalValues = Array.from(conditionNamesToDeleteSet).map(
        toApiDeleteConditionalValueData,
      );
    }
  ...
  const conditionNamesToRecreateSet = new Set(conditionNamesToRecreate ?? []);
  ...
  // Branch precedence per condition name:
  // 1) delete list always wins (even if same name appears in update/recreate)
  // 2) update list applies next; for recreate set we emit delete+create
  // 3) everything else is treated as create
  conditionValue.forEach((value, conditionName) => {
    if (conditionNamesToDeleteSet.has(conditionName)) {
      toUpdateConditionalValues.push(toApiDeleteConditionalValueData(conditionName));
      return;
    }
    if (conditionNamesToUpdateSet.has(conditionName)) {
      if (conditionNamesToRecreateSet.has(conditionName)) {
        // API v2 does not support renaming/updating existing conditional
        // values in-place for this staged-edit case; recreate explicitly.
        toUpdateConditionalValues.push(toApiDeleteConditionalValueData(conditionName));
  ...
  // Delete names can include conditions that are not present in submitted map
  // (for example user removed them from form completely). We still need to
  // emit explicit delete operations for those names.
  conditionNamesToDeleteSet.forEach((conditionName) => {
    if (conditionValue.has(conditionName)) {
      return;
    }
    toUpdateConditionalValues.push(toApiDeleteConditionalValueData(conditionName));
  });
