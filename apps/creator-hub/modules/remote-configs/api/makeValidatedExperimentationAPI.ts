...
import type {
  ...
  PreviewRolloutResponse as DangerousPreviewRolloutResponse,
...
import {
  ...
  isDangerousConditionalConfigEntry,
  isDangerousRuleConfigEntry,
  isDangerousRuleOrderingConfigEntry,
  toAPIConfigEntryValue as toAPIConfigEntryValueForConfigsProductType,
  toValidExperimentConfigEntry,
  toValidConfigEntryStaged,
  toValidConfigEntryValue,
  toValidConditionalEntryIdentity,
  toValidConditionRule,
  toValidRuleOrdering,
...
import type {
  ...
  InternalV1UniversesUniverseIdExperimentExperimentIdRolloutapplyPostRequest,
  ValidPreviewRolloutResponse,
  ValidApplyRolloutResponse,
  ValidRolloutConflict,
  ValidRolloutNewCondition,
...
class PreviewNeedsPublishedEntriesError extends Error {
  constructor(message: string) {
    super(message);
    this.name = 'PreviewNeedsPublishedEntriesError';
  }
}
...
export const toValidGetExperimentResponse = (
  ...
  switch (experimentProductType) {
    case ExperimentProductType.Configs: {
      ...
      const validVariants = dangerousVariants.map((dangerousVariant) => {
        ...
        return {
          ...
          configEntry: toValidExperimentConfigEntry(configEntry),
        };
      });
      validExperimentConfiguration = {
    ...
    default: {
      ...
      throw new Error(`Invalid experiment product type: ${String(exhaustiveCheck)}`);
    }
  }
          ...
          configEntry: {
            ...
            entryValue:
              variant.configEntry.entryValue == null
                ? null
                : toAPIConfigEntryValueForConfigsProductType(variant.configEntry.entryValue),
          },
        })),
      };
      experimentConfiguration = {
...
export const toValidPreviewRolloutResponse = (
  dangerousResponse: DangerousPreviewRolloutResponse,
  publishedEntries?: ValidConfigEntryInfo[],
  publishedRuleOrdering?: ValidRuleOrdering,
  publishedRules?: Map<string, ValidConditionRule>,
): ValidPreviewRolloutResponse => {
  const entries: ValidConfigEntryStaged[] = [];
  const newConditions: ValidRolloutNewCondition[] = [];
  const rules: Map<string, ValidConditionRule> = new Map();
  const deletedRuleConditionKeys: string[] = [];
  ...
  const pendingCurrentConditionValuesByKey: Map<
    string,
    Map<string, ValidConfigEntryValue>
  > = new Map();
  const pendingDeletedConditionNamesByKey: Map<string, Set<string>> = new Map();
  let currentRuleOrdering: ValidRuleOrdering | undefined;
  let stagedRuleOrdering: ValidRuleOrdering | undefined;

  for (const rawEntry of dangerousResponse.entries ?? []) {
    const dangerousOverrideEntry = rawEntry.overrideEntry?.entry;
    if (!dangerousOverrideEntry?.key) {
      continue;
    }

    if (isDangerousRuleOrderingConfigEntry(dangerousOverrideEntry)) {
      stagedRuleOrdering = toValidRuleOrdering(dangerousOverrideEntry);
      const currentValue: unknown = rawEntry.currentValue;
      if (
        currentValue != null &&
        typeof currentValue === 'object' &&
        'conditionOrder' in currentValue
      ) {
        const currentOrder: unknown = currentValue.conditionOrder;
        if (Array.isArray(currentOrder) && currentOrder.every((item) => typeof item === 'string')) {
          currentRuleOrdering = { conditionOrder: currentOrder };
        }
      }
      continue;
    }
    ...
    if (isDangerousConditionalConfigEntry(dangerousOverrideEntry)) {
      const { key, conditionName } = toValidConditionalEntryIdentity(dangerousOverrideEntry);
      const currentConditionValue =
        rawEntry.currentValue != null ? toValidConfigEntryValue(rawEntry.currentValue) : undefined;
      const isConditionDeleted =
        rawEntry.isDeleted === true || dangerousOverrideEntry.entryValue == null;
      const existingIndex = nonDeletedEntryIndexByKey.get(key);

      if (isConditionDeleted) {
        if (existingIndex === undefined) {
          // No base entry in this response yet; defer the deletion so it can be
          // reconciled against published entries after the loop.
          if (currentConditionValue) {
            const pending =
              pendingCurrentConditionValuesByKey.get(key) ??
              new Map<string, ValidConfigEntryValue>();
            pending.set(conditionName, currentConditionValue);
            pendingCurrentConditionValuesByKey.set(key, pending);
          }

          const pendingDeletedConditionNames =
            pendingDeletedConditionNamesByKey.get(key) ?? new Set<string>();
          pendingDeletedConditionNames.add(conditionName);
          pendingDeletedConditionNamesByKey.set(key, pendingDeletedConditionNames);
        } else {
          // The base entry is already present, so apply the deletion in place.
          // Tracking it as pending would spuriously reconstruct a duplicate entry
          // for this key during published-entry hydration.
          const existing = entries[existingIndex];
          if (!existing || existing.isDeleted) {
            throw new Error('Conditional staged entry requires a non-deleted base entry');
          }
          if (currentConditionValue) {
            existing.currentConditionValue ??= new Map();
            existing.currentConditionValue.set(conditionName, currentConditionValue);
          }
          existing.overrideEntry.entry.conditionValue?.delete(conditionName);
        }

        continue;
      }

      const entryValue = toValidConfigEntryValue(dangerousOverrideEntry.entryValue);
      const currentValue = currentConditionValue;

      if (existingIndex !== undefined) {
        const existing = entries[existingIndex];
        if (existing && !existing.isDeleted) {
          existing.overrideEntry.entry.conditionValue ??= new Map();
          existing.overrideEntry.entry.conditionValue.set(conditionName, entryValue);
          if (currentValue) {
            existing.currentConditionValue ??= new Map();
            existing.currentConditionValue.set(conditionName, currentValue);
          }
        }
      } else {
        const pending =
          pendingConditionValuesByKey.get(key) ?? new Map<string, ValidConfigEntryValue>();
        pending.set(conditionName, entryValue);
        pendingConditionValuesByKey.set(key, pending);
        if (currentValue) {
          const pendingCurrent =
            pendingCurrentConditionValuesByKey.get(key) ?? new Map<string, ValidConfigEntryValue>();
          pendingCurrent.set(conditionName, currentValue);
          pendingCurrentConditionValuesByKey.set(key, pendingCurrent);
        }
      }
      continue;
    }
    ...
    if (isDangerousRuleConfigEntry(dangerousOverrideEntry)) {
      const conditionKey = dangerousOverrideEntry.key;
      if (!conditionKey) {
        throw new Error('Rule config type requires a key');
      }

      if (rawEntry.isDeleted) {
        deletedRuleConditionKeys.push(conditionKey);
        continue;
      }

      rules.set(conditionKey, toValidConditionRule(dangerousOverrideEntry));

      if (rawEntry.currentValue == null) {
        newConditions.push({
          configKey: conditionKey,
    ...
    if (
      dangerousOverrideEntry.configType != null &&
      !isDangerousValueConfigEntry(dangerousOverrideEntry)
    ) {
      if (rawEntry.currentValue == null) {
        const conditionName =
          dangerousOverrideEntry.conditionalValueData?.conditionName ?? dangerousOverrideEntry.key;
        newConditions.push({
          configKey: dangerousOverrideEntry.key,
          conditionName,
        });
        if (dangerousOverrideEntry.entryValue != null) {
          const entryValue = toValidConfigEntryValue(dangerousOverrideEntry.entryValue);
          const existingIndex = nonDeletedEntryIndexByKey.get(dangerousOverrideEntry.key);
          if (existingIndex !== undefined) {
            const existing = entries[existingIndex];
            if (existing && !existing.isDeleted) {
              existing.overrideEntry.entry.conditionValue ??= new Map();
    ...
    const validated = toValidConfigEntryStaged(rawEntry);
    if (validated && !validated.isDeleted) {
      const { key } = validated.overrideEntry.entry;
      const pendingConditions = pendingConditionValuesByKey.get(key);
      if (pendingConditions) {
        validated.overrideEntry.entry.conditionValue = validated.overrideEntry.entry.conditionValue
          ? new Map([...validated.overrideEntry.entry.conditionValue, ...pendingConditions])
          : pendingConditions;
        pendingConditionValuesByKey.delete(key);
      }
      const pendingCurrentConditions = pendingCurrentConditionValuesByKey.get(key);
      if (pendingCurrentConditions) {
        validated.currentConditionValue = validated.currentConditionValue
          ? new Map([...validated.currentConditionValue, ...pendingCurrentConditions])
          : pendingCurrentConditions;
        pendingCurrentConditionValuesByKey.delete(key);
      }
      const pendingDeletedConditionNames = pendingDeletedConditionNamesByKey.get(key);
      pendingDeletedConditionNames?.forEach((deletedConditionName) => {
        validated.overrideEntry.entry.conditionValue?.delete(deletedConditionName);
      });
      if (pendingDeletedConditionNames) {
        pendingDeletedConditionNamesByKey.delete(key);
      }
  ...
  if (
    pendingConditionValuesByKey.size > 0 ||
    pendingCurrentConditionValuesByKey.size > 0 ||
    pendingDeletedConditionNamesByKey.size > 0
  ) {
    if (publishedEntries === undefined) {
      const keysWithoutBase = Array.from(
        new Set<string>([
          ...pendingConditionValuesByKey.keys(),
          ...pendingCurrentConditionValuesByKey.keys(),
          ...pendingDeletedConditionNamesByKey.keys(),
        ]),
      ).join(', ');
      throw new PreviewNeedsPublishedEntriesError(
    ...
    pendingKeys.forEach((key) => {
      ...
      const pendingConditionValues =
        pendingConditionValuesByKey.get(key) ?? new Map<string, ValidConfigEntryValue>();
      const pendingCurrentConditionValues = pendingCurrentConditionValuesByKey.get(key);
      const pendingDeletedConditionNames = pendingDeletedConditionNamesByKey.get(key);
      const mergedConditionValues = publishedEntry.conditionValue
        ...
        : pendingConditionValues;
      pendingDeletedConditionNames?.forEach((conditionName) => {
        mergedConditionValues.delete(conditionName);
      });
      const currentConditionValues = pendingCurrentConditionValues
        ? new Map<string, ValidConfigEntryValue>([
            ...(publishedEntry.conditionValue ?? new Map<string, ValidConfigEntryValue>()),
            ...pendingCurrentConditionValues,
          ])
        : publishedEntry.conditionValue;
      ...
      entries.push({
        ...
        ...(currentConditionValues ? { currentConditionValue: currentConditionValues } : {}),
        overrideEntry: {
          entry: {
            ...publishedEntry,
    ...
    Array.from(pendingDeletedConditionNamesByKey.keys()).forEach((key) => {
      if (pendingConditionValuesByKey.has(key)) {
        return;
      }
      pendingCurrentConditionValuesByKey.delete(key);
      pendingDeletedConditionNamesByKey.delete(key);
  ...
  const conflicts: ValidRolloutConflict[] = (dangerousResponse.conflicts ?? []).reduce<
    ValidRolloutConflict[]
  >((acc, c) => {
    if (c.configKey != null && c.conditionName != null && c.kind != null) {
      acc.push({
  ...
  const previewHash = dangerousResponse.previewHash;
  if (!previewHash) {
    handleError('previewHash is required in PreviewRolloutResponse');
  }

  if (!currentRuleOrdering && stagedRuleOrdering && publishedRuleOrdering) {
    currentRuleOrdering = publishedRuleOrdering;
  }
  ...
  return {
    ...
    newConditions,
    rules,
    currentRules: publishedRules,
    deletedRuleConditionKeys,
    currentRuleOrdering,
    stagedRuleOrdering,
    conflicts,
    previewHash,
...
const makeValidatedExperimentationAPI = (
  ...
  configsApi?: ValidRemoteConfigAPI,
): ValidExperimentationAPI => {
  return {
    ...
    v1UniversesUniverseIdExperimentExperimentIdRolloutpreviewPost: async (
      requestParameters: InternalV1UniversesUniverseIdExperimentExperimentIdRolloutpreviewPostRequest,
    ): Promise<ValidPreviewRolloutResponse> => {
      const dangerousResponse =
      ...
      try {
        let result = toValidPreviewRolloutResponse(dangerousResponse);
        const needsPublishedConfig =
          (result.stagedRuleOrdering != null && result.currentRuleOrdering == null) ||
          result.rules.size > 0 ||
          result.deletedRuleConditionKeys.length > 0;
        if (configsApi && needsPublishedConfig) {
          const published = await configsApi.v1ConfigurationsUniversesUniverseIdLatestGet({
            universeId: requestParameters.universeId,
          });
          result = toValidPreviewRolloutResponse(
            dangerousResponse,
            published.entries,
            published.ruleOrdering,
            published.rules,
          );
        }
        return result;
      } catch (error) {
        if (error instanceof PreviewNeedsPublishedEntriesError && configsApi) {
          const published = await configsApi.v1ConfigurationsUniversesUniverseIdLatestGet({
            universeId: requestParameters.universeId,
          });
          return toValidPreviewRolloutResponse(
            dangerousResponse,
            published.entries,
            published.ruleOrdering,
            published.rules,
    ...
    v1UniversesUniverseIdExperimentExperimentIdRolloutapplyPost: async (
      requestParameters: InternalV1UniversesUniverseIdExperimentExperimentIdRolloutapplyPostRequest,
    ): Promise<ValidApplyRolloutResponse> => {
      const dangerousResponse =
