...
import type { V1ConfigurationsUniversesUniverseIdLatestGetRequest } from '@rbx/client-universe-configs-web-api/v1';
...
type UseLatestConfigurationsResult = {
  ...
  /** unsorted entries for current version. contains all published entries for PublishAs */
  unsortedEntries: ValidConfigEntryDetail[];
...
export const useLatestConfigurations = ({
  ...
  const { data, isLoading, isError } = useQuery({
    ...
    queryFn: async ({ client: cacheClient }) => {
      if (!universeId) {
        throw new Error('Should not be enabled without universeId');
      }
  ...
  return useMemo(() => {
    return {
      entries: unsortedEntries
        ...
        : emptyEntries,
      unsortedEntries: unsortedEntries ?? emptyEntries,
