...
const featureToMenuItem = <T,>(
  ...
  return {
    ...
    key: feature.key,
    title: feature.displayName ?? translate(feature.nameKey), // TODO @mryumae: ensure translation is correct once public
    adornment: feature.adornment,
    subItems: feature.subFeatures?.map((subFeature) => featureToMenuItem(subFeature, translate)),
    content: feature,
  };
};
...
function Features<T>({
  ...
  const menuItems: MenuItem<Feature<T>>[] = useMemo(
    () => features.map((feature) => featureToMenuItem(feature, translate)),
    [features, translate],
  );
  const handleSelectFeature = useCallback(
    async (item: MenuItem<Feature<T>>) => {
      const selectedFeature = item.content;
