...
import type { MockServerSignalValues } from '@rbx/client-matchmaking-api/v1';
...
import { PageLoading, YoutubeVideo } from '@modules/miscellaneous/components';
...
> = ({ isEditingConfiguration, currentConfiguration }) => {
  ...
  }, [activeExperiment, currentConfiguration.id]);
  ...
  useEffect(() => {
    setDefaultSignalsMap(getDefaultSignalsWeightsMap(currentConfiguration?.defaultSignals));
  }, [currentConfiguration?.defaultSignals]);
  ...
  const handleFormSave = useCallback(
    async (selectedPlaceIds: number[]) => {
      ...
      if (isApplyToPlacesSuccessful && isConfigUpdateSuccessful) {
        void router.push(dashboard.getCustomMatchmakingDashboardUrl(Number(gameDetails?.id)));
