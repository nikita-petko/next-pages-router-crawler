...
const RevShareProposeFlowContainer: FunctionComponent<RevShareProposeFlowContainerProps> = ({
  ...
  const [submissionError, setSubmissionError] = useState<ClassifiedRevShareMutationError | null>(
    null,
  );
  const [isRefreshingSubmissionError, setIsRefreshingSubmissionError] = useState(false);
  ...
  const handleRefreshSubmissionError = useCallback(async () => {
    if (isRefreshingSubmissionErrorRef.current) {
      return;
    }
    isRefreshingSubmissionErrorRef.current = true;
    setIsRefreshingSubmissionError(true);
    try {
      await refetchManagerAgreementsRef.current({ throwOnError: true });
      onExitRef.current();
    } catch {
