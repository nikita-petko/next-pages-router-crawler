...
import type { ManagedProduct, ManagedProductWithRevenue } from '../../manage-items/types';
...
function parseProductWithRevenue(mv: RAQIV2MetricValue): ParsedProductWithRevenue | null {
  ...
  const revenueLast30Days = revenueLast30DaysRaw ? Number(revenueLast30DaysRaw) : null;
 ...
 */
function useTopManagedProducts({
  universeId,
  limit,
  ...
  const topProductsRequest: RAQIV2UIQueryRequest = useMemo(
    ...
    [universeId, limit, aggregationDurationMs],
  );
...
export default useTopManagedProducts;
