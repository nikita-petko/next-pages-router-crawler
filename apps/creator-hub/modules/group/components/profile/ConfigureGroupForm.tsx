...
import type {
  ConfigureGroupFormType,
  GroupConfiguration,
  GroupIcon,
} from '../../ConfigureGroupTypes';
...
const ConfigureGroupForm: FunctionComponent<React.PropsWithChildren<ConfigureGroupFormProps>> = ({
  ...
  const dirtyFieldsRequiringConfirmation = fieldsRequiringConfirmation.filter((field) => {
    if (field === FieldRequiringConfirmation.ASSET_PRIVACY_DEFAULT_RESTRICTED) {
      ...
      return dirtyFields[field] && !getValues(field);
    }
  ...
  const configureGroupIcon = useCallback(
    async (groupId: number, icon: GroupIcon) => {
      ...
      try {
        await groupsClient.patchGroupIcon({
          groupId,
          files: icon.file,
        });
      } catch (errRes) {
        ...
        throw new Error(errorMsgKey, { cause: errRes });
      }
    },
    [dirtyFields],
  );
  ...
  const configureGroupName = useCallback(
    async (groupId: number, name: string) => {
      ...
      try {
        await groupsClient.patchGroupName({
          groupId,
          request: {
            name,
          },
  ...
  const configureGroupDescription = useCallback(
    async (groupId: number, description: string) => {
      ...
      try {
        await groupsClient.patchGroupDescription({
          groupId,
          request: {
            description,
          },
      ...
      try {
        ...
        const updatePromises = previousSocialLinks.map(async (prevLink) => {
          ...
          if (newLink === undefined) {
            return groupsClient.deleteGroupSocialLink({
              groupId,
        ...
        const creationPromises = socialLinks.map((newLink) => {
          ...
          if (!newLink.title || !newLink.url || !newLink.type) {
            return undefined;
          }
        ...
        await Promise.all([...updatePromises, ...creationPromises]);
      } catch (errRes) {
        let errorMsgKey = 'Error.UnknownError';
        const err = await tryParseResponseError(errRes);
        if (err) {
  ...
  const configureOwner = useCallback(
    async (groupId: number, owner: User) => {
      ...
      try {
        await groupsClient.postGroupOwner({
          groupId,
          changeOwnerRequest: {
            userId: owner.id,
          },
  ...
  const mergeErrorMsg = useCallback(
    (
      ...
      if (iconRes.status === 'rejected') {
        failureReasonKey =
          iconRes.reason instanceof Error ? iconRes.reason.message : 'Error.UnknownError';
        errorFields.push(translate('Label.Icon'));
      }
      if (nameRes.status === 'rejected') {
        failureReasonKey =
          nameRes.reason instanceof Error ? nameRes.reason.message : 'Error.UnknownError';
        errorFields.push(translate('Label.OrganizationName'));
      }
      if (descriptionRes.status === 'rejected') {
        failureReasonKey =
          descriptionRes.reason instanceof Error
            ? descriptionRes.reason.message
            : 'Error.UnknownError';
        errorFields.push(translate('Label.Description'));
      }
      if (socialLinksRenameRes.status === 'rejected') {
        failureReasonKey =
          socialLinksRenameRes.reason instanceof Error
            ? socialLinksRenameRes.reason.message
            : 'Error.UnknownError';
        errorFields.push(translate('Label.SocialLinks'));
      }
      if (assetPrivacyDefaultRes.status === 'rejected') {
        failureReasonKey =
          assetPrivacyDefaultRes.reason instanceof Error
            ? assetPrivacyDefaultRes.reason.message
            : 'Error.UnknownError';
  ...
  const handleFormSubmit: SubmitHandler<ConfigureGroupFormType> = useCallback(
    async (data) => {
      ...
      if (rejected.length === settledPromises.length && iconRes.status === 'rejected') {
        const message =
          iconRes.reason instanceof Error ? iconRes.reason.message : 'Error.UnknownError';
        setFormSubmissionErrorMsg(translate(message));
        return;
      }
