...
import type FilteredUserType from '../interface/FilteredUserType';
...
const AddPayoutCreator: FunctionComponent<AddPayoutCreatorProps> = ({
  ...
  const filterUsers = useCallback(
    async (results: Array<User>) => {
      ...
      const membershipPromises = filteredResults.map((result) =>
        result.id ? checkGroupMembership(groupId, result.id) : Promise.resolve(false),
      );
      ...
      const definedGroupMemberUserIds = groupMemberUserIds.filter((id) => id !== undefined);
  ...
  const isMaxRecipientsReached = payouts.length >= MaxRecurringPayoutCount;
  ...
  return (
    <Grid container direction='column'>
      <Grid
        ...
        <Grid item XSmall>
          <DebouncedUserSearch
            ...
            isDisabled={isMaxRecipientsReached}
          />
        </Grid>
        <Grid item>
          <Grid container alignItems='center' justifyContent='flex-start' wrap='nowrap'>
            ...
            <IconButton
              ...
              color='secondary'
              disabled={disabled || !creator || percentage === '' || isMaxRecipientsReached}
      ...
      {isMaxRecipientsReached && (
        <FormHelperText>
          {translate('Label.SearchMaxUsersReached' /* in TranslationNamespace.Payouts */, {
            maxUsers: MaxRecurringPayoutCount.toString(),
          })}
        </FormHelperText>
      )}
    </Grid>
