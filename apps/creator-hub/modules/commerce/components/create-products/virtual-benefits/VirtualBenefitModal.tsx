...
import { CommerceGrantableModel, GrantableType } from '@rbx/clients/commerceApi/v1';
import { useFormContext } from 'react-hook-form';
import { developerProductsClient } from '@modules/clients';
...
export interface VirtualBenefitModalProps {
...
const VirtualBenefitModal: FunctionComponent<React.PropsWithChildren<VirtualBenefitModalProps>> = ({
  productImageId,
  productImageUrl,
  productName,
    ...
    switch (selectedBenefitType) {
      ...
      case GrantableType.DeveloperProduct:
        return [
          {
            grantableAssetId:
              watch('grantableAssetId') === undefined || watch('grantableAssetId') === ''
                ? 'draft-dev-product'
                : watch('grantableAssetId'),
            name: watch('name'),
            description: watch('description'),
            grantableType: selectedBenefitType,
            ...
            {selectedBenefitType === GrantableType.DeveloperProduct && (
              <GrantableSelection
                ...
                onSelect={async (grantableItem) => {
                  ...
                  const res = await developerProductsClient.getDeveloperProductFromTargetId({
                    developerProductId: Number.parseInt(grantableItem.grantableAssetId ?? '0', 10),
                  });
                  setValue('grantableAssetId', res.id?.toString() ?? '');
