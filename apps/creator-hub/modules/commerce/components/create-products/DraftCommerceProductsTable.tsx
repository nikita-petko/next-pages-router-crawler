...
import { CommerceFailureReason, CommerceProductModel } from '@modules/clients/commerce';
import { Thumbnail2d, ThumbnailTypes } from '@rbx/thumbnails';
import { CommerceGrantableModel, ProductStatusType } from '@rbx/clients/commerceApi/v1';
import { getResponseFromError } from '@modules/clients/utils';
import { GrantableType, InventoryType } from '@rbx/clients/commerceApi';
...
import { VirtualBenefitFormType } from './virtual-benefits/types';
...
const DraftCommerceProductsTable: FunctionComponent<DraftCommerceProductsTableProps> = ({
  ...
  const developerProductsToDisplay = useMemo(() => {
    ...
    return productsWithDeveloperProduct.reduce((acc, product) => {
      const developerGrantables = product.commerceGrantables.filter(
        (grantable) => grantable.grantableType === GrantableType.DeveloperProduct,
      );
      return [...acc, ...developerGrantables];
  ...
  const { control, watch, setValue, reset } = useForm<SelectedState>({
  ...
  const getGrantableThumbnail = (grantable: CommerceGrantableModel) => {
    let thumbnailTargetType = grantableTypeToThumbnailType[grantable.grantableType!];
            ...
            <Grid container alignItems='center'>
              ...
              <Grid item XSmall={2}>
                <IconButton
                  ...
                  onClick={(event: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
                    event.stopPropagation();
                    onUpdateBenefit(commerceProduct.id, []); // Remove the benefit by updating with an empty array
                  }}
                                ...
                                <Tooltip arrow placement='top' title={translate('Action.Delete')}>
                                  <IconButton
                                    ...
                                    onClick={(
                                      ...
                                      event.stopPropagation();
                                      onClickDelete(commerceProductId);
                                    }}>
                                    <DeleteOutlinedIcon />
                                  </IconButton>
                                </Tooltip>
