...
const useTaxIdValidationState = (
  ...
  return useMemo((): TaxIdValidationState => {
    ...
    if (isMasked) return TaxIdValidationState.Masked;
    if (!hasValidType && !hasValue) return TaxIdValidationState.Empty;
    if (hasValidType && !hasValue) return TaxIdValidationState.TypeOnly;
    if (!hasValidType && hasValue) return TaxIdValidationState.IdOnly;
