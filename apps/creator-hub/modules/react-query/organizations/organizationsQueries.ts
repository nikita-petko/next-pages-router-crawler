...
import type { UpdateRolePositionRequestModel } from '@rbx/client-organizations-service-api/v1';
...
export function useUpdateRolePosition() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      if (previousRoleId) {
        body.previousRoleId = previousRoleId;
      }
      if (nextRoleId) {
        body.nextRoleId = nextRoleId;
      }
  ...
  return useQuery({
    ...
    queryFn:
      typeof universeId !== 'undefined' && universeId !== uninitializedUniverseId
        ...
        : skipToken,
    // Temporary fix to avoid unnecessary refetches on page mounts, owners should replace this with proper stale time
