...
async function requestJson<TResponse>(
  ...
  // eslint-disable-next-line @typescript-eslint/no-unsafe-type-assertion -- API boundary; callers specify the expected response shape
...
export const BOOSTED_CORES_KEY = 'BoostedCoresEnabledPlaceIds';

const EXTENDED_SERVICES_REPOSITORY: CreatorConfigsPublicApiRepository = 'ExtendedServicesConfig';

export async function publishExtendedServicesCoresPlaceIds(
  universeId: string,
  placeIds: number[],
  message = 'Updated boosted cores place ids through creator-hub',
): Promise<CreatorConfigsPublicApiPublishDraftResponse> {
  const opts: CreatorConfigsPublicApiRequestOptions = {
    universeId,
    repository: EXTENDED_SERVICES_REPOSITORY,
  };
  const { draftHash } = await updateDraft(opts, {
    entries: { [BOOSTED_CORES_KEY]: placeIds },
  });
  if (!draftHash) {
    throw new Error('Missing draftHash');
  }
  return publishDraft(opts, {
    draftHash,
    message,
...
export async function getExtendedServicesCoresPlaceIds(universeId: string): Promise<number[]> {
  const repo = await getConfigRepositoryValues({
    universeId,
    repository: EXTENDED_SERVICES_REPOSITORY,
  });
  const raw = repo.entries?.[BOOSTED_CORES_KEY];
