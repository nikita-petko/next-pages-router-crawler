import { getBEDEV2ServiceBasePath, getResponseFromError } from '@modules/clients/utils';
import datastoresV2Client from '@modules/clients/dataStoresManager';
import { Configuration } from '@rbx/clients';
import { CloudApi, DataStoreEntry, DataStoreEntryStateEnum } from '@rbx/clients/openCloud';
import { V2CloudProtos } from '@rbx/open-cloud';
import unifiedLoggerClient from '@modules/eventStream/unifiedLoggerClient';
import { DataStoreList, EntryList, DataStore, UniverseStorage, DataStoreState } from './types';
import {
...
const ocBasePath = getBEDEV2ServiceBasePath('user');
const ocConfiguration = new Configuration({
...
export async function listEntries(
  ...
  // TODO: Refactor return type to use DataStoreEntry in @rbx/clients/openCloud package instead
  return {
    ...
    try {
      ...
      storageResponse.dataStores.forEach((dataStore) => {
        storageMap.set(dataStore.name, {
          bytesTotalPermannet: dataStore.bytesTotalPermanent,
      ...
      const dataStoreList = listResponse.dataStores.map(
        (dataStore) =>
          ({
            name: dataStore.name,
            totalSizeBytes: getDisplayBytes(storageMap.get(dataStore.name)?.bytesTotalPermannet),
