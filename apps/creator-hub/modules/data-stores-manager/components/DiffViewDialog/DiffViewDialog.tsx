      ...
      <Select
        ...
        {availableVersions.entries.map((version) => (
          <MenuItem
            ...
            {version.revisionCreateTime ? formatTimestamp(version.revisionCreateTime, locale) : ''}
          </MenuItem>
        ))}
...
const DiffViewDialog: FunctionComponent<DiffViewDialogProps> = ({
  ...
  const loadMoreVersions = useCallback(async () => {
    if (isLoadingMoreVersions || !availableVersions.cursor) {
      return;
    }
  ...
  const clearDiffStateAndLoad = useCallback(
    (versionId: string, isLeft: boolean) => {
      ...
      void loadVersionData(versionId, isLeft);
    },
    [loadVersionData],
  );
  ...
  return (
    <Dialog open fullWidth maxWidth='Large' className={dialogContainer}>
      <DialogTitle className={dialogTitle}>{translate('Label.CompareVersions')}</DialogTitle>
      <DialogContent>
