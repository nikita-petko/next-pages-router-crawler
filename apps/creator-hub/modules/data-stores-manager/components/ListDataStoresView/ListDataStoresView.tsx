...
const ListDataStoresView: FunctionComponent<DataStoresListProps> = ({
  ...
  const setupPagination = useCallback((listDataStores: DataStore[], startCursor?: string) => {
  ...
  const resetListDataStoresState = useCallback(
    async (
      ...
      const effectiveShowDeleted = showDeletedParam ?? showDeleted;
  ...
  const searchBoxValueChanged = useCallback(
  ...
  const searchBoxKeyDown = useCallback(
    ...
    [dataStoresSearchBoxValue, resetListDataStoresState, showDeleted, rowsPerPage],
  );
  ...
  const handleSearchCancel = () => {
    ...
    if (currentPrefix && currentPrefix !== '') {
      void resetListDataStoresState('', showDeleted, 0, true, '');
    }
  ...
  const handleToggleShowDeleted = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (!isChecked && getResponseFromError(error)?.status === StatusCodes.GATEWAY_TIMEOUT) {
        setShowDeleted(true);
        showToast(translate('Message.HideDeletedTimeout'), true);
        return;
      }
                ...
                <TableRow
                  ...
                  <TableCell
                    ...
                    <Typography variant='caption' component='p'>
                      {dataStore.name}
                      {dataStore.state === DataStoreState.DELETED && dataStore.expireTime
              ...
              <TablePagination
                ...
                onPageChange={(e, newPageNumber) => {
                  ...
                  if (newPageNumber < dataStoresPage || targetPageCursor !== undefined) {
                    void handleDataStoresPageChange(newPageNumber);
                  }
                }}
