...
const copyToClipboard = (data: string | null | undefined) => {
  if (data !== undefined) {
    void navigator.clipboard.writeText(data ?? 'null');
  }
};
  ...
  const entryData = (() => {
    ...
    try {
      ...
      if (typeof actualValue === 'string') {
        try {
          const parsed: unknown = JSON.parse(actualValue);
      ...
      try {
        ...
        // eslint-disable-next-line typescript/no-base-to-string -- last-resort fallback when stringify throws (e.g. circular refs)
