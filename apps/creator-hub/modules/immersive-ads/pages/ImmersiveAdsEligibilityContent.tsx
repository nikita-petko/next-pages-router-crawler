...
import { ModerationStatus, useEligibility } from '../contexts/EligibilityContext';
...
const ImmersiveAdsEligibilityContent = () => {
  const {
    classes: {
      descriptionStyle,
  ...
  useEffect(() => {
    if (typeof window === 'undefined' || universeId <= 0) {
      return;
    }
    const isFinalStatus =
      experienceGuidelinesModerationStatus === ModerationStatus.Approved ||
    ...
    if (isFinalStatus) {
      ...
      // oxlint-disable-next-line react/react-compiler -- syncing local state from cache on mount
  ...
  const redirectToContentSettingsPage = () => {
    void router.push(`/dashboard/creations/experiences/${universeId}/configure`);
  };

  const redirectToExperienceQuestionnairePage = () => {
    void router.push(`/dashboard/creations/experiences/${universeId}/experience-questionnaire`);
  };
  ...
  return (
    <div className={eligibilityContainer}>
      ...
      <div className={eligibilityActionsContainer}>
        ...
        />
        <EligibilityRow
          eligibilityInfo={translate('Description.VerifyContentMaturityQuestionnaire')}
