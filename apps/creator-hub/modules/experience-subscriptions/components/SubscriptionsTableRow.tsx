...
import { ProductStatusType, CurrencyType } from '@rbx/clients/developerSubscriptionsApi';
...
} from '@modules/creations/experienceSubscriptions';
import type { CreationData } from '@modules/creations';
import { useQueryClient } from '@tanstack/react-query';
import getConfigureSubscriptionLink from './common/constants';
import { SubscriptionCreatorDetails } from '../types/SubscriptionCreatorDetails';
  ...
  const displayPrice = (() => {
    ...
    if (subscription.price) {
      return numberFormatter(
        (subscription.price.units ?? 0) + (subscription.price.cents ?? 0) / 100.0,
        'currency',
      );
    }
    return '-';
