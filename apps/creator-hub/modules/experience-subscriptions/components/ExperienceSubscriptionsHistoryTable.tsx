...
import { ExperienceSubscriptionsTableRowSpec } from '../types/ExperienceSubscriptionsTableTypes';
...
function ExperienceSubscriptionsHistoryTable() {
  ...
  const formatPrice = useCallback((row: ExperienceSubscriptionsTableRowSpec) => {
    if (row.price) {
      return numberFormatter((row.price.units ?? 0) + (row.price.cents ?? 0) / 100.0, 'currency');
    }
