...
export const RemovalRequestsURL = '/dashboard/rights-manager/removal-requests';
...
const RemovalRequestsContainer = () => {
  ...
  const handleTabChange = useCallback(
    (_: React.SyntheticEvent<Element, Event>, value: ListRemovalRequestsTabs) => {
      setQueryParams({ activeTab: (value as ListRemovalRequestsTabs).toString() });
    },
    [setQueryParams],
  );
          ...
          <Typography variant='body1' color='secondary'>
            {translateHTML('Label.RegistrationDetails', [
              ...
              },
              {
                ...
                content(chunks) {
                  return (
                    <Link href={accountURL} passHref legacyBehavior>
                      <MuiLink color='primary'>{chunks}</MuiLink>
                    </Link>
                  );
                },
