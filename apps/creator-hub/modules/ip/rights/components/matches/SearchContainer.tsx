...
import { ClaimItemDiscoveredFromEnum, ErrorResponseAppErrorCodeEnum } from '@rbx/client-rights/v1';
...
const SearchContainer = ({ onSubmit, cart, currentSource }: SearchContainerProps) => {
  ...
  const searchQuery = useMemo(
    () => ({
      text: searchSubmission.searchText,
      imageId: searchSubmission.imageId,
    }),
    [searchSubmission.searchText, searchSubmission.imageId],
  );

  const {
    response: { data, fetchNextPage, hasNextPage, error, isFetching, isSuccess },
    reset: resetSearch,
  } = useSearchQuery(searchSubmission.searchType, searchQuery, source, filter);
