...
import { Asset, AssetPrivacy, AssetType, Creator } from '@rbx/clients/assetsUploadApi/v1';
...
export const uploadAsset = async ({
  ...
  while (Date.now() - startTime < maxTime) {
    // eslint-disable-next-line no-await-in-loop -- we want to have only one outoging call at a time
    ...
    // eslint-disable-next-line no-await-in-loop -- we're leveraging await to set the poll interval
