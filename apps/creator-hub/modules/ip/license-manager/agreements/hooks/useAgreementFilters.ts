...
import { creatorAgreementTabsConfig } from '../../creatorAgreements/constants';
...
const useGetAgreementFilters = (isCreator?: boolean) => {
  ...
  const userSelectedFilter = isAgreementFilterKey(tabParam) ? tabParam : undefined;
  ...
  const countsQuery = useGetAgreementCounts(
    isCreator ?? false,
    tabsConfig.map((tab) => tab.keyName),
  );

  const filtersWithCounts = useMemo(() => {
    return tabsConfig.map((filter) => {
      const count = countsQuery.data?.get(filter.keyName) ?? 0;
      return {
        ...filter,
        count,
      };
  ...
  const effectiveSelectedFilter = useMemo((): AgreementFilterKeys | undefined => {
