...
/* oxlint-disable typescript/no-floating-promises -- fire-and-forget query invalidation in mutation onSuccess handlers */
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query';
import type {
  AgreementCandidatePromotionType,
  CancellationReason,
} from '@rbx/client-content-licensing-api/v1';
import { ListingStatus } from '@rbx/client-content-licensing-api/v1';
...
export const usePromoteAgreementCandidateMutation = () => {
  ...
  const enableIpPlatformConditionalOffers = settings.enableIpPlatformConditionalOffers;
...
/**
 * IPH activates an agreement in Accepted state after deep-scan validates IP usage on the target.
 */
export const useActivateAcceptedAgreementMutation = () => {
  const queryClient = useQueryClient();
  const { account } = useCurrentAccountContext();
  const accountId = account?.id;
  ...
  return useMutation({
    mutationFn: async (agreementId: string) => {
      ...
      return contentLicensingClient.activateAcceptedAgreement(accountId, agreementId);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: AGREEMENTS_QUERY_KEY });
    },
    onError: (error, agreementId) => {
      captureException(error, {
        tags: { module: 'license-manager', operation: 'activateAcceptedAgreement' },
        extra: { agreementId, accountId },
      });
    },
  });
