...
import type { ContentStandardAnswer } from '@rbx/client-content-licensing-api/v1';
import {
...
export const useLicenseQuery = (licenseId: string, options?: { enabled?: boolean }) => {
  const { account } = useCurrentAccountContext();
  const accountId = account?.id;
  ...
  return useQuery({
    ...
    enabled: !!accountId && !!licenseId && enabledByCaller,
  });
};
...
export const useCreateIpListingMutation = ({
  ...
  return useMutation({
    ...
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: IP_LISTINGS_QUERY_KEY });
      onSuccess?.();
    },
    onError: (error) => {
...
export const useAddLicenseMutation = ({
  ...
  return useMutation({
    mutationFn: async (params: CreateLicenseParams) => {
      ...
      } else if (params.contentStandardsDocumentId !== undefined) {
        documentId = params.contentStandardsDocumentId ?? undefined;
      }
...
export const useUpdateLicenseMutation = ({
  ...
  return useMutation({
    mutationFn: async ({ licenseId, contentStandardsDocument, ...params }: UpdateLicenseParams) => {
      ...
      let documentId: string | undefined = params.contentStandardsDocumentId ?? undefined;
      ...
      const updateLicenseRequest: LicensesUpdateLicenseRequest = {
        ...
        contentStandardAnswers: params.contentStandardAnswers ?? [],
        licenseDuration: params.licenseDuration,
        ...(params.licenseTerms !== undefined && { licenseTerms: params.licenseTerms }),
      };
