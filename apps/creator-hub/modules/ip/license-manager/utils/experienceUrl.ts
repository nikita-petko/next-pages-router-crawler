...
export function useExperiencePrivacyCheck({
  ...
  const universeDetailsQuery = useQuery({
    queryKey: ['universeDetails', universeId, audiencesReplacementOn],
    queryFn: async () => {
      if (universeId == null) {
        return null;
      }
  ...
  const { data, isPending, isFetching } = universeDetailsQuery;
  const hasUniverseDetails = data != null;
  const audienceIsPublic = hasUniverseDetails
    ? (data.audiences?.includes(Audience.Public) ?? false)
    : false;
  const isPrivate = hasUniverseDetails
    ? audiencesReplacementOn
      ? !audienceIsPublic
      : data.privacyType === 'Private'
    : false;
  const isArchived = hasUniverseDetails && data.isArchived === true;
  const isDraft = hasUniverseDetails && !audiencesReplacementOn && data.isActive === false;
  const isNotPermitted = isPrivate || isArchived || isDraft;
  const isPublic = audiencesReplacementOn
    ? hasUniverseDetails && audienceIsPublic && data.isArchived !== true
    : hasUniverseDetails &&
      data.privacyType === 'Public' &&
      data.isArchived !== true &&
      data.isActive !== false;
  const isCheckingPrivacy = universeId != null && !hasUniverseDetails && (isPending || isFetching);
