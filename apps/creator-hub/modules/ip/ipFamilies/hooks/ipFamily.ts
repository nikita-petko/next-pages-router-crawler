      ...
      if (imageAssetIds.length > 0) {
        contents.push(
          ...imageAssetIds.map((imageAssetId) => ({
            contentType: IPContentContentTypeEnum.Asset,
            contentValue: imageAssetId.toString(),
            ipFamilyId: ipFamily.id,
            isPrimary: false,
            robloxAssetType: SupportedRobloxAssetTypeEnum.Image,
          })),
        );
      }
...
interface CreateIpContentsAndAddToIpFamilyParams {
  ...
  assets?: { assetId: number; citation: string }[];
  ...
  return useMutation({
      ...
      if (data.assets && data.assets.length > 0) {
        contents.push(
          ...data.assets.map((asset) => ({
            contentType: IPContentContentTypeEnum.Asset,
            contentValue: asset.assetId.toString(),
            ipFamilyId: data.ipFamilyId,
            isPrimary: false,
    ...
    onError: (error, variables) => {
      captureException(error, {
        tags: {
          module: 'ip-families',
          operation: 'createIpContentsAndAddToFamily',
        },
...
interface CreateTrademarkParams {
  ipFamilyId: string;
...
/**
 * A hook that creates a trademark IP content
 */
export const useCreateTrademarkMutation = () => {
  const { account } = useCurrentAccountContext();
  const queryClient = useQueryClient();
  const accountId = account?.id;
  const { user } = useRobloxAuthentication();
  ...
  return useMutation({
    mutationFn: async (data: CreateTrademarkParams) => {
      if (!accountId) {
        throw new Error('Missing account ID');
      }
      if (!user) {
      ...
      const documentIds = await createDocuments(data.documents);
      ...
      if (data.image !== undefined) {
        ...
        [imageAssetId] = uploadedAssetIds;
      }

      const trademarkContent: TrademarkContent = {
      ...
      const ipContent: IPContent =
        data.image !== undefined
          ? {
              ...
              contentValue: imageAssetId?.toString() ?? '',
              robloxAssetType: SupportedRobloxAssetTypeEnum.Image,
              imageCaption: data.trademarkName,
              isPrimary: false,
              trademarkContent,
            }
          : {
              ipFamilyId: data.ipFamilyId,
              contentType: IPContentContentTypeEnum.Text,
              contentValue: data.trademarkName,
              isPrimary: false,
              trademarkContent,
            };

      await rightsClient.createIpContent({
        accountId,
        iPContent: ipContent,
      });
    },
    ...
    onError: (error, variables) => {
      captureException(error, {
        tags: {
          module: 'ip-families',
          operation: 'createTrademarkIpContent',
        },
        extra: { accountId, ipFamilyId: variables.ipFamilyId },
      });
    },
...
type IPContentsByAccountFilter =
  | {
      key: 'ip_content_type';
      value: IPContentContentTypeEnum;
    }
  | {
      key: 'asset_type';
      value: SupportedRobloxAssetTypeEnum;
    };

export const useListAllIpContentsByAccount = (params: { filter?: IPContentsByAccountFilter }) => {
  const { account } = useCurrentAccountContext();
  const accountId = account?.id;
  const filter = params.filter ? `${params.filter.key}="${params.filter.value}"` : '';
  return useQuery({
    queryKey: LIST_IP_CONTENTS_BY_ACCOUNT(accountId ?? '', filter),
    queryFn: async () => {
      if (!accountId) {
        throw new Error('Missing account ID');
      }
...
export const useUpdateIpContentMutation = ({
  ...
  return useMutation({
    ...
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: IP_CONTENTS_QUERY_KEY });
      onSuccess?.();
    },
    onError: (error, variables) => {
      captureException(error, {
