...
const IpContentDisplay: FunctionComponent<IpContentDisplayProps> = ({ ipContent }) => {
  ...
  switch (ipContent.contentType) {
    ...
    case IPContentContentTypeEnum.Asset:
      return (
        <>
          {ipContent.trademarkContent && (
            <Typography variant='body2' className={classes.imageCaption}>
              {ipContent.imageCaption}
            </Typography>
          )}
...
const ViewIpContentDetails = ({ ipContent, onEdit, onArchive }: ViewIpContentDetailsProps) => {
  ...
  const ipContentTypeTranslationKey = getIpContentTypeTranslationKey(ipContent, {
    granularTrademark: true,
  });

  const getLocaleText = (): string => {
    switch (ipContent.contentType) {
      case IPContentContentTypeEnum.Text:
        return ipContent.locale ? translate(getTranslationKeyFromLocale(ipContent.locale)) : '';
      case IPContentContentTypeEnum.Image:
      case IPContentContentTypeEnum.Asset:
        ...
        <Grid container direction='column' spacing={3}>
          ...
          {ipContent.status === IPContentStatusEnum.Rejected && ipContent.statusReason && (
            <Grid item>
              ...
              <Typography variant='body2'>
                {getIpContentStatusReason(ipContent.statusReason, ipContent, translate)}
              </Typography>
            </Grid>
          )}
          ...
          <Grid item>
            ...
            <Typography variant='body2' className={classes.blockDisplay}>
              {ipContentTypeTranslationKey ? translate(ipContentTypeTranslationKey) : ''}
            </Typography>
          </Grid>
          ...
          {ipContent.trademarkContent && (
            <Grid item>
              <Typography variant='body2' className={classes.fieldLabel}>
                {translate('Heading.TrademarkDetails')}
              </Typography>
              <Typography variant='body2' className={classes.blockDisplay}>
                {translate('Label.RegistrationCountryValue', {
                  registrationCountry: ipContent.trademarkContent.registrationCountry ?? '',
                })}
              </Typography>
              <Typography variant='body2' className={classes.blockDisplay}>
                {translate('Label.RegistrationNumberValue', {
                  registrationNumber: ipContent.trademarkContent.registrationNumber ?? '',
                })}
              </Typography>
            </Grid>
          )}
  ...
  switch (ipContent.contentType) {
    ...
    case IPContentContentTypeEnum.Asset:
      if (ipContent.robloxAssetType === SupportedRobloxAssetTypeEnum.Image) {
        ...
        archiveMessage = translate('Description.ArchiveAsset');
      }
      break;
    case IPContentContentTypeEnum.Text:
...
const IpContentDetailsDrawer: FunctionComponent<IpContentDetailsDrawerProps> = ({
  ...
  const [previousDefaultMode, setPreviousDefaultMode] = useState(defaultMode);
  if (open !== previousOpen || defaultMode !== previousDefaultMode) {
    setPreviousOpen(open);
    setPreviousDefaultMode(defaultMode);
    if (open) {
      setMode(defaultMode);
    }
  }
