...
type CommunicationSettingsFormProps = {
  ...
  updateCommunicationSetting(value: AtfsCommunicationSetting): void;
...
> = ({
  ...
  }, [universeId, setIsContentRestricted]);
      ...
      <Grid container item XSmall={12}>
        <Typography variant='body1'>
          {settings.isContentMaturityRenameEnabled
            ...
            : translate('Label.StrongLanguageDescription')}
        </Typography>
      </Grid>
      <Grid className={styles.channelContainer} container direction='column'>
        <Grid item className={styles.option} key={communicationSetting.category}>
          ...
          {!isContentRestricted ? (
            <Tooltip
              title={
                settings.isContentMaturityRenameEnabled
                  ? translate('Label.NonRestrictedGameNotAllowedToEnableStrongLanguage')
                  : translate('Label.Under17GameNotAllowedToEnableStrongLanguage')
              }
              placement='top'
              enterTouchDelay={0}
              leaveTouchDelay={3000}>
