...
const useTableContexts = ({
  ...
  const intervalMs = millisecondsInInterval(seriesIntervalMeaning);
  ...
  const { pageStartTime, pageEndTime } = useMemo(() => {
    ...
    switch (cohortOrder) {
      ...
      case TableSortOrder.desc: {
        ...
        switch (tableContext.granularity) {
          case RAQIV2MetricGranularity.OneWeek:
            ...
            start += millisecondsInInterval(DailyTimeSeriesAlignedToUTCMidnight);
            break;
      ...
      default: {
        ...
        throw new Error(`Unhandled table sort order: ${exhaustiveCheck}`);
      }
    }
