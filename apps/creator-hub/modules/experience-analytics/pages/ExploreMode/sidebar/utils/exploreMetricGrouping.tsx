...
export type MetricGroup = {
  ...
  metrics: Array<{ metric: TExploreModeMetrics; label: FormattedText }>;
};
...
export function getGroupSortIndex(key: string, namespace?: string): number {
  const idx = GROUP_SORT_ORDER.findIndex(
    (entry) => entry.key === key && (namespace === undefined || entry.namespace === namespace),
  );
...
export function buildMetricGroupMenuItems(
  groups: MetricGroup[],
  selectedMetric: TExploreModeMetrics | null,
  ...
  groups.forEach((group, idx) => {
    ...
    items.push(
      <div role='group' key={`section-${group.groupKey.key}`} className='padding-small'>
        {showCategoryLabels && (
          <div
            role='none'
