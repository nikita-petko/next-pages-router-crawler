...
import { PageLoading, Link } from '@modules/miscellaneous/common';
import { FailureView } from '@modules/miscellaneous/common/components/FailureView';
import { PreviewSubmissionsResponse } from '@modules/clients';
...
const QuestionnairePreviewV2: FunctionComponent<QuestionnairePreviewV2Props> = ({
  ...
  // We need to build a dumb truth table based on the types of activie policies
  // on the experience. We use a bitwise approach to generate the table since
  // bits map to a truth table 1:1
  ...
  const formatVersionLabel = (index: number): string => {
    return `${index + 1}`;
  };
                      ...
                      {policies.map((policy) => (
                        <TableCell key={policy.id}>
                          <span className='text-body-medium'>
                            {formatPolicyValue(version[policy.id] as boolean)}
                          </span>
                        </TableCell>
                      ))}
                    </TableRow>
