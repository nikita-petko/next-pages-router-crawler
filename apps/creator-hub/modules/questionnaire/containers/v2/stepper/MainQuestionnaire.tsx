...
const MainQuestionnaire: FunctionComponent<MainQuestionnaireProps> = ({
  ...
  const isMainQuestionnaireComplete = useMemo(() => {
    if (!remainingSectionsQuestionnaire) {
      return false;
    }
  ...
  const getSaveStatusLabel = () => {
    if (isSaving) {
      return translate('Progress.Saving');
    }
    if (saveStatus === 'saved') {
      return translate('Label.Saved');
    }
    if (saveStatus === 'unsaved') {
      return translate('Label.UnsavedChanges');
    }
    return null;
  };
