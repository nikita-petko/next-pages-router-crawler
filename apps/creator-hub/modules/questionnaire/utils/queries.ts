import {
  experienceQuestionnaireClient,
  experienceQuestionnaireV2Client,
  iarcActivityServiceClient,
...
import { ValidateResponseResponse } from '@rbx/clients/experienceQuestionnaire/v1';
import { deepValidateQuestionnaire, validateAnswers } from './validationHelpers';
import { ValidatedAnswer, ActivityEvent } from '../interfaces/types';
...
type TUseMutationOptions<D = unknown, T = void> = Omit<
  UseMutationOptions<D, Error, T, unknown>,
  ...
  return useQuery<GetLatestAdditionalSubmissionResponse | null>({
    ...
    queryFn: async () => {
      try {
        ...
        // eslint-disable-next-line no-console -- Warning on 404 for latest submission
        console.warn(
