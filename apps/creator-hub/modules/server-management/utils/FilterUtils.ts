...
export function validateFilter(filter: GameServerFilters | undefined): boolean {
  ...
  const noServerStatuses = Object.values(filter.serverStatus).every((v) => !v);
  const serverStatusesInvalid = allServerStatuses || noServerStatuses;
...
export function isFilterRestartViable(
  ...
  if (filter.engineVersion && filter.engineVersion.length > 0) return false;

  if (filter.serverType) {
    const allServerTypesDefault = Object.values(filter.serverType).every((v) => v === true);
    if (!allServerTypesDefault) return false;
  }
  if (filter.serverStatus) {
    const allServerStatusDefault = Object.values(filter.serverStatus).every((v) => v === true);
    if (!allServerStatusDefault) return false;
  }
