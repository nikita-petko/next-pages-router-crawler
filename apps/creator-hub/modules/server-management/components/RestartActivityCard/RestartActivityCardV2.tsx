...
import { ListRestartStatusesResponse, RestartStatus } from '@rbx/clients/serverManagementService';
...
const RestartActivityCardV2: React.FC<RestartActivityCardV2Props> = () => {
  ...
  const { paginatedUpdates, totalUpdates } = useMemo(() => {
    ...
    const updates = entries.map(([id, status]) => ({ id, ...status })).reverse();
  ...
  return (
    <div>
      ...
      <RestartProgressSheet
        onOpenChange={(open) => {
          if (!open) setSelectedUpdateIndex(-1);
        }}
