...
const RestartServersModalV2: FunctionComponent<RestartServersModalProps> = ({
  ...
  }, [apiResponse, effectivePlaces, restartOptions.restartOutdatedOnly, translate, filter]);
  ...
  const payloadValidation = (() => {
    if (
      !restartOptions.bleedOffEnabled ||
      !restartOptions.customPayloadEnabled ||
    ...
    let parsed: unknown;
    try {
      parsed = JSON.parse(restartOptions.customPayload);
    } catch {
      return { valid: false, error: translate('RestartServersModal.CustomPayloadInvalidJson') };
    }
    if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed)) {
      return { valid: false, error: translate('RestartServersModal.CustomPayloadMustBeObject') };
    }
    const byteLength = new TextEncoder().encode(restartOptions.customPayload).length;
    if (byteLength > VALIDATION_CONSTANTS.CUSTOM_PAYLOAD_MAX_BYTES) {
      return {
        valid: false,
        error: translate('RestartServersModal.CustomPayloadTooLarge', {
          current: byteLength.toString(),
          max: VALIDATION_CONSTANTS.CUSTOM_PAYLOAD_MAX_BYTES.toString(),
        }),
      };
    }
    return { valid: true, error: '' };
  })();
  const isPayloadValid = payloadValidation.valid;

  const canConfirm =
    isBleedOffValid && isPayloadValid && impactData.serversImpacted > 0 && !impactData.error;
          ...
          {settings.serverManagementCustomPayloadReady && (
            <Grid item>
              <Grid container className='flex flex-col gap-medium'>
                ...
                <div className='flex flex-col gap-small' style={{ paddingLeft: 36 }}>
                  <TextArea
                    ...
                    rows={4}
                    isDisabled={
                      !restartOptions.bleedOffEnabled || !restartOptions.customPayloadEnabled
                    }
                    hasError={!isPayloadValid}
                    textareaStyle={{ maxWidth: 600 }}
                  />
                  {!isPayloadValid && payloadValidation.error && (
