...
export type ValidProductType = Exclude<ProductType, 'Invalid'>;
...
export const productKeyToIdentifier = (key: string): ProductIdentifier => {
  const [productType, productId] = key.split('#');
  const parsedProductType: ProductType =
    productType === 'GamePass' || productType === 'DeveloperProduct' ? productType : 'Invalid';
  return {
