...
import type { GetExperimentationMetadataResponse } from '@rbx/client-price-experimentation-api/v1';
...
export const useGetExperimentationMetadata = () => {
  const { mediumDateFormatter } = useFormatters();

  const { data, isPending, isError } = useQuery({
    queryKey: [getExperimentationMetadataQueryKey, mediumDateFormatter.resolvedOptions().locale],
    queryFn: async () => {
      const response = await priceExperimentationApi.getExperimentationMetadata();
