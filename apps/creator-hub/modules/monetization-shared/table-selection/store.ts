...
export type SelectionStoreParams<K extends string | number, T> = {
   ...
   *  When provided, non-selectable items are excluded from toggle/bulk actions,
   *  shown as disabled in hooks, and auto-evicted from the selection on data sync. */
...
export function createSelectionStore<K extends string | number, T>(
  ...
  function computeViewableSelectedCount(selectedMap: Map<K, T>, viewableIds: Set<K>): number {
    if (viewableIds.size === 0) return 0;

    let count = 0;
    // eslint-disable-next-line no-restricted-syntax -- for-of is better
    for (const id of selectedMap.keys()) {
      if (viewableIds.has(id)) count += 1;
    }
  ...
  return {
        ...
        if (itemsChanged) {
          ...
          if (selectable !== defaultSelectable && selectedMap.size > 0) {
            ...
            for (const [id] of selectedMap) {
              ...
              if (freshItem && !selectable(freshItem)) {
    ...
    toggleItem: (item: T, isChecked: boolean) => {
      ...
      if (isChecked && !wasAlreadySelected && isSelectionUnderLimit && selectable(item)) {
        nextMap.set(id, item);
    ...
    toggleBulk: (isChecked: boolean) => {
      ...
      if (isChecked) {
        targetArray.forEach((item) => {
          if ((limit === undefined || nextMap.size < limit) && selectable(item)) {
