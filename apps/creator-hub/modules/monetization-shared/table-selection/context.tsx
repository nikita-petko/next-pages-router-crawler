...
export function useTableSelectionStoreInstance<K extends string | number, T>(
  ...
  useLayoutEffect(() => {
    ...
    // oxlint-disable-next-line react-hooks/exhaustive-deps -- this is correct
...
export function TableSelectionProvider<K extends string | number, T>({
  ...
  // useLayoutEffect ensures the store is updated before children render and calculate stats.
  useLayoutEffect(() => {
    if (!currentPage && !items && !mode && !limit && !disabled) {
      return;
    }
...
> {
  ...
  // oxlint-disable-next-line typescript/no-unsafe-type-assertion -- intended type narrowing
