...
import { retryWithRateLimitBackoff } from './retry';
...
/**
 * Subset of `UseInfiniteQueryResult` that `useLoadInitialPages` depends on.
 * Consumers pass the return value of a domain-specific "count" hook
 * (e.g. `useCountDeveloperProducts`, `useCountGamePasses`).
 */
export type UseLoadInitialPagesCountResult = {
  /** Running total of items across all fetched pages (`undefined` while the first page is in flight). */
  data: number | undefined;
  /** `true` while the first page is still loading. */
  isLoading: boolean;
  /** `true` if the first page fetch failed. */
  isLoadingError: boolean;
  /** `true` if there are more pages available to fetch. */
  hasNextPage: boolean;
  /**
   * Fetches the next page. Must support `throwOnError` so the hook can detect
   * failures. Resolves to the post-fetch `{ data, hasNextPage }` so prefetch
   * loop can advance off the fetch result rather than the accumulated `count`.
   */
  fetchNextPage: (options?: { throwOnError: boolean }) => Promise<{
    /** Accumulated count after this fetch (the query's `select` output). */
    data: number | undefined;
    /** `true` if more pages remain after this fetch. */
    hasNextPage: boolean;
  }>;
};

export type UseLoadInitialPagesResult = {
  /** `true` when the count resolves to exactly 0. */
  readonly isEmpty: boolean;
  /** `true` once the accumulated count meets or exceeds `initialTotal`. */
  readonly hitInitialTotal: boolean;
  /** `true` while pages are still being pre-fetched (includes the first page load). */
  readonly isInitialLoading: boolean;
  /** `true` if any page fetch failed with a non-silent error. */
  readonly isInitialError: boolean;
};

/**
 * Default retries per page fetch in the prefetch loop. A 429 backs off
 * (`Retry-After` / exponential) between attempts; see
 * {@link retryWithRateLimitBackoff}. Override per call via `useLoadInitialPages`.
 */
const DEFAULT_PAGE_FETCH_RETRIES = 3;

/**
 * Shared hook that pre-fetches pages from an infinite query until a target
 * item count (`initialTotal`) is reached or there are no more pages.
 *
 * It is designed to sit on top of a domain-specific "count" hook that wraps
 * `useInfiniteQuery` with `useInfiniteCounter` as its `select`.
 *
 * @param countResult - The result object from the count hook (a subset of `UseInfiniteQueryResult<number>`).
 * @param initialTotal - Target number of items to pre-fetch. Use `Infinity` to load all pages.
 * @param pageFetchRetries - Retries per page fetch on a 429 (defaults to {@link DEFAULT_PAGE_FETCH_RETRIES}).
 * @returns Loading, error, empty, and threshold-hit flags.
 *
 * @example
 * ```ts
 * // Developer products
 * function useLoadInitialDeveloperProducts(params, options) {
 *   const countResult = useCountDeveloperProducts(params, options);
 *   return useLoadInitialPages(countResult, params.initialTotal ?? 3000);
 * }
 *
 * // Game passes
 * function useLoadInitialGamePasses(params, options) {
 *   const countResult = useCountGamePasses(params, options);
 *   return useLoadInitialPages(countResult, params.initialTotal ?? 3000);
 * }
 *
 * // Load ALL pages
 * function useLoadAllGamePasses(params, options) {
 *   const countResult = useCountGamePasses(params, options);
 *   return useLoadInitialPages(countResult, Infinity);
 * }
 * ```
 */
export function useLoadInitialPages(
  countResult: UseLoadInitialPagesCountResult,
  initialTotal: number,
  pageFetchRetries: number = DEFAULT_PAGE_FETCH_RETRIES,
): UseLoadInitialPagesResult {
  const { data: count, isLoading, isLoadingError, hasNextPage, fetchNextPage } = countResult;

  const [isInitialLoading, setIsInitialLoading] = useState<boolean>(false);
  const [isInitialError, setIsInitialError] = useState<boolean>(false);
  // Prevents a second prefetch loop from starting while one is already running.
  const isPrefetchingRef = useRef<boolean>(false);

  // Pages off each fetch's resolved `{ data, hasNextPage }` rather than the
  // accumulated `count`
  const prefetchUntilTarget = useCallback(
    async (loadedCount: number, hasMorePages: boolean) => {
      ...
      try {
        ...
        let canFetchMore = hasMorePages;
        while (currentCount < initialTotal && canFetchMore) {
          // Retries the page fetch; a 429 backs off so rate limits don't abort
          // the prefetch.
          const result = await retryWithRateLimitBackoff(
    ...
    [fetchNextPage, initialTotal, pageFetchRetries],
  );
  ...
  useEffect(() => {
    ...
    if (count < initialTotal && hasNextPage) {
      void prefetchUntilTarget(count, hasNextPage);
    } else {
      setIsInitialLoading(false);
    }
  }, [count, isLoading, initialTotal, hasNextPage, prefetchUntilTarget]);
