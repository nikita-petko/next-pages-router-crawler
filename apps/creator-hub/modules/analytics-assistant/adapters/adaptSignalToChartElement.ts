...
import type { ItemMonetizationProductTypes, RAQIV2QueryFilter } from '@modules/clients/analytics';
import { MonetizationProductTypes } from '@modules/clients/analytics';
...
import { tableConfigFunnelsProgressionByUserRealtime } from '@modules/experience-analytics-shared/constants/chart-configs/PredefinedTableConfigLiterals';
import type {
  ChartConfig,
...
import type { QueryFilter } from '../../clients/analytics/analyticsRAQIShared';
import {
...
import type { SummaryReportUISignal, SummaryReportSignalMetric } from '../types/AssistantUISignal';
import { NonRAQIAssistantMetric } from '../types/AssistantUISignal';
...
const filterSupportedBreakdowns = (
  ...
  const supportedDimensions = getChartConfiguratorDimensions()[metric] || [];
  ...
  if (signal.signalMetadata) {
    switch (signal.signalMetadata.signalType) {
      ...
      case SignalType.RatioKpiChangeAttribution: {
        // Ratio attribution signals only render configured metrics; all others intentionally skip breakdowns.
...
const filterSupportedFilters = (
  ...
  const supportedDimensions = getChartConfiguratorDimensions()[metric] || [];
  return filters.filter((f) => supportedDimensions.includes(f.dimension));
};
...
const buildBaseChartContext = (
  ...
  timeSpec: { rangeType: RAQIV2DateRangeType.Custom, startTime: startDate, endTime: endDate },
...
type AssistantChartType = Exclude<ChartConfig['chartType'], ChartType.MultipleMetricSpline>;

const getAssistantChartType = (chartType: ChartType | undefined): AssistantChartType => {
  switch (chartType) {
...
const getAcquisitionChartConfig = (
  ...
  if (!isAcquisitionBySourceMetric) {
    return undefined;
  }
    ...
    props: {
      tableContext: {
        ...
        timeSpec: {
          ...
          startTime: signal.startDate,
          endTime: signal.endDate,
        },
        filter,
...
const createDynamicChartElement = (
  ...
  const chartType = getAssistantChartType(displayConfig.exploreModeChartType);
  ...
  const chartConfig = {
    ...
    chartHeight: AssistantCanvasChartHeight,
    inRoundedComparisonChipContext: true,
    // The chart type is narrowed above; this keeps the generated object aligned with ChartConfig's discriminated union.
  } as ChartConfig;
...
const adaptSignalToChartElement = (
  ...
  switch (signal.signalType) {
    ...
    case SignalType.RatioKpiChangeAttribution: {
      // Ratio attribution signals only render configured metrics; all others intentionally produce no chart.
      // eslint-disable-next-line @typescript-eslint/switch-exhaustiveness-check
      switch (signal.metric) {
        case RAQIV2Metric.ForwardD1Retention: {
          if (!signal.signalMetadata) {
