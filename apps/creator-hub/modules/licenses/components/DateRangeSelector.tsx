...
export interface DateRangeSelectorProps {
  ...
  disablePast?: boolean;
  /**
   * When `disablePast` is true, shifts the earliest selectable date to today (local midnight) plus
   * this many whole calendar days. Ignored when `disablePast` is false. Default 0.
   */
  minimumLeadDaysBeforeSelectableDate?: number;
  ...
  (
    {
      ...
      onChange,
      disablePast = false,
      minimumLeadDaysBeforeSelectableDate = 0,
      disableFuture = false,
    ...
    const minDate = useMemo(() => {
      if (disablePast) {
        ...
        if (minimumLeadDaysBeforeSelectableDate > 0) {
          const shifted = new Date(base);
          shifted.setDate(shifted.getDate() + minimumLeadDaysBeforeSelectableDate);
      ...
      return toMidnight(d);
    }, [disablePast, maxLookbackMonths, minimumLeadDaysBeforeSelectableDate]);
