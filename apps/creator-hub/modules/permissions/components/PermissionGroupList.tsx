...
/**
 * Builds the set of permission IDs that should be hidden based on feature flags.
 * Each entry gates a single permission: when its flag (`enabled`) is disabled,
 * the permission ID is added to the hidden set.
 */
const getHiddenPermissionIds = (
  gatedPermissions: { enabled: boolean; permissionId: string }[],
): Set<string> => {
  const hidden = new Set<string>();

  gatedPermissions.forEach(({ enabled, permissionId }) => {
    if (!enabled) {
      hidden.add(permissionId);
    }
  });
...
const PermissionGroupList: FunctionComponent<PermissionGroupListProps> = ({ creator, entity }) => {
  ...
  >();
  const [permissionsSource, setPermissionsSource] = useState(initialPermissions);
  const [showSaveConfirmation, setShowSaveConfirmation] = useState(false);
  const [memberCount, setMemberCount] = useState<number | undefined>();

  if (initialPermissions !== permissionsSource) {
    setPermissionsSource(initialPermissions);
    setEditedPermissions(undefined);
  }

  const permissionData = editedPermissions ?? initialPermissions;
  ...
  const onPermissionChange = useCallback(
    (permissionId: string, isGranted: boolean) => {
      setEditedPermissions((prevEditedPermissions) => {
        const prevPermissionData = prevEditedPermissions ?? initialPermissions;
        if (!prevPermissionData) {
          return prevEditedPermissions;
        }
  ...
  const handleSelectAllCheckboxChange = useCallback(
    (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      setEditedPermissions(newPermissionsData);
    },
    [displayMessage, permissionData, initialPermissions],
  );
  ...
  const maybePromptAndPersist = useCallback(() => {
    ...
    if (showConfirmationOnSave) {
      ...
      void persistPermissions(permissionData);
    }
  }, [
    displayMessage,
  ...
  const closeDialogAndPersist = useCallback(() => {
    ...
    setShowSaveConfirmation(false);
    void persistPermissions(permissionData);
  }, [setShowSaveConfirmation, persistPermissions, permissionData, displayMessage]);
  ...
  const revokeAllPermissions = useCallback(() => {
    ...
    setEditedPermissions(newPermissionsData);
    void persistPermissions(newPermissionsData);
  }, [permissionData, initialPermissions, persistPermissions, displayMessage]);
        ...
        {metadata.map((permissionGroup) => {
          const filtered = {
            ...
            permissions: permissionGroup.permissions.filter(
              (p) => !hiddenPermissionIds.has(p.permissionId),
            ),
          };
          return (
            <PermissionGroup
