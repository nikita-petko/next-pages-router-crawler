...
const ToggleRegionalPricingButton = memo(
  ({
    ...
    const action = useBulkToggleAction(
      (product: DeveloperProductConfig) => product.isRegionalPricingEnabled,
    );
...
const CreateOrBulkActionButton = memo(
  ({
    ...
    if (!hasSelection) {
      return <CreateDeveloperProductButton universeId={universeId} />;
    }
...
function DeveloperProductsActionBar({
  universeId,
  onBulkAction,
  isBulkActionDisabled,
  isBulkActionPending,
...
export default memo(DeveloperProductsActionBar);
