...
import { visibleAssetIdInPersonalizationEnabled as visibleAssetIdInPersonalizationFlag } from '@generated/flags/creatorAnalytics';
...
import {
  ...
  type TPersonalizedThumbnailsTableColumnKey,
} from '@modules/experience-analytics-shared/constants/PersonalizedThumbnailsTableConfig';
import usePersonalizedThumbnailsRAQIV2RowData from '@modules/experience-analytics-shared/hooks/usePersonalizedThumbnailsRAQIV2RowData';
...
const isPersonalizedThumbnailsTableEntry = (
  entry: [string, CellDataType],
): entry is [TPersonalizedThumbnailsTableColumnKey, CellDataType] =>
  entry[0] in PersonalizedThumbnailsTableColumnConfigs;
...
const PersonalizedThumbnailsTable: FC<PersonalizedThumbnailsTableProps> = ({
  ...
  const { value: visibleAssetIdInPersonalizationFlagValue } = useFlag(
    visibleAssetIdInPersonalizationFlag,
  );
  const visibleAssetIdInPersonalizationEnabled = visibleAssetIdInPersonalizationFlagValue ?? false;
  ...
  const tableRowData = useMemo(() => {
    ...
    rowsWithRAQIOnlyData.forEach(({ isTotalRow, cellData }) => {
      ...
      if (thumbnailAssetCell?.type !== ColumnType.Text) {
        logAnalyticsError(
          `thumbnailAssetCell type ${thumbnailAssetCell?.type ?? 'missing'} is not text type`,
        );
        return;
      }

      const rowData = new Map<TPersonalizedThumbnailsTableColumnKey, CellDataType>(
        Array.from(cellData).filter(isPersonalizedThumbnailsTableEntry),
      );
