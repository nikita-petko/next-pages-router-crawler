...
import {
  groupsClient,
  GroupRoleDetailResponse,
  translationRolesClient,
  TranslatorType,
...
const TranslatorManagementProvider: FunctionComponent<React.PropsWithChildren<unknown>> = ({
  children,
}) => {
  ...
  const addTranslators = useCallback(
    async (translatorIds: number[], translatorType: TranslatorType) => {
      ...
      try {
        /* eslint-disable no-await-in-loop, no-restricted-syntax -- API doesn't support concurrent requests */
        ...
        /* eslint-enable no-await-in-loop, no-restricted-syntax -- re-enable after sequential loop */
        await loadTranslatorsData(false);
      } catch (e) {
