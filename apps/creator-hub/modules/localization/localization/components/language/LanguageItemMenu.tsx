...
const LanguageItemMenu: FunctionComponent<React.PropsWithChildren<LanguageItemMenuProps>> = ({
  ...
  const infoAutoTranslationSwitchText = isInfoAutoTranslationOn
    ? translate('Label.TurnAutoTranslationOff')
    : translate('Label.TurnAutoTranslationOn');
  const imageAutoTranslationSwitchText = isImageAutoTranslationOn
    ? translate('Label.TurnAutoTranslationOff')
    : translate('Label.TurnAutoTranslationOn');
  const {
  ...
  const [isAutoTranslationInfoDeleteDialogOpen, setIsAutoTranslationInfoDeleteDialogOpen] =
    useState<boolean>(false);
  const [isAutoTranslationImageDeleteDialogOpen, setIsAutoTranslationImageDeleteDialogOpen] =
    useState<boolean>(false);
  ...
  const handleAutoTranslationImageSwitchOn = () => {
    handleSwitchImageAutoTranslation(languageCode, true);
    trackerClient.sendEvent(
      switchOnAutomaticTranslationEventModel(
        CreatorDashboardSource.LocalizationAutoTranslationImage,
        gameId,
        languageCode,
      ),
    );
  ...
  const handleAutoTranslationImageSwitchOff = () => {
    handleSwitchImageAutoTranslation(languageCode, false);
    setIsAutoTranslationImageDeleteDialogOpen(false);
    trackerClient.sendEvent(
      switchOffAutomaticTranslationEventModel(
        CreatorDashboardSource.LocalizationAutoTranslationImage,
        gameId,
  ...
  const handleAutoTranslationImageDeleteOpenDialog = () => {
    setIsAutoTranslationImageDeleteDialogOpen(true);
  };
  const handleAutoTranslationImageDeleteCloseDialog = () => {
    setIsAutoTranslationImageDeleteDialogOpen(false);
    trackerClient.sendEvent(
      switchOffAutomaticTranslationEventModel(
        CreatorDashboardSource.LocalizationAutoTranslationImage,
  ...
  return (
    <>
      ...
      <Menu
        ...
        {enableImageTranslationEnrollment &&
          ...
          isImageAutoTranslationAvailable && (
            <ListItemButton
              onClick={
                isImageAutoTranslationOn
                  ? handleAutoTranslationImageDeleteOpenDialog
                  : handleAutoTranslationImageSwitchOn
              }>
              <ListItemText>
                <Typography variant='captionHeader' display='block'>
                  {imageAutoTranslationSwitchText}
                </Typography>
                <Typography variant='captionBody'>
        ...
        <Dialog
          maxWidth='Medium'
          open={enableImageTranslationEnrollment && isAutoTranslationImageDeleteDialogOpen}>
          <DialogTemplate
            color='destructive'
            cancelText={translate('Action.CloseATDeleteDialog')}
            confirmText={translate('Action.TurnOff')}
            onCancel={handleAutoTranslationImageDeleteCloseDialog}
            onConfirm={handleAutoTranslationImageSwitchOff}
            title={translate('Label.AutomaticTranslationOff') + languageName}
            content={autoTranslationSwitchOffDescription}
          />
        </Dialog>
