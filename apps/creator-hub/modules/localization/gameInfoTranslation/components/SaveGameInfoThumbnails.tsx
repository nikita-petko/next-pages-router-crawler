...
> = ({ imageList, isLoading, onSaveSuccess }) => {
  ...
  const updateGameThumbnailAltText = useCallback(
    async (thumbnailId: string, altText?: string) => {
      try {
        ...
        throw new Error(`Failed to update alt text for ${thumbnailId}`);
      }
    },
  ...
  const handleClickSave = useCallback(async () => {
    try {
      ...
      // eslint-disable-next-line no-restricted-syntax -- There are some problems in the backend: When firing requests simutaneously not all the image is saved while they all return 200. Waiting for the backend to solve the problem
      for (const imageToAdd of imagesToUpdate.imagesToAdd) {
        try {
          // eslint-disable-next-line no-await-in-loop -- TODO: refactor to make recursive calls
          ...
          throw new Error(`Failed to save ${imageToAdd.name}`);
        }
      }
      // eslint-disable-next-line no-restricted-syntax -- TODO: refactor to make recursive calls
      for (const imageToDelete of imagesToUpdate.imagesToDelete) {
        try {
          ...
          throw new Error(`Failed to delete ${imageToDelete.key}`);
        }
      }
      // eslint-disable-next-line no-restricted-syntax -- TODO: refactor to make recursive calls
