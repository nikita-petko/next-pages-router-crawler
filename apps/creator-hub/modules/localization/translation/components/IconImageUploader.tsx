...
import type { FileRejections } from '@modules/miscellaneous/components/uploaders/components/SingleImageUploader';
import SingleImageUploader from '@modules/miscellaneous/components/uploaders/components/SingleImageUploader';
...
const IconImageUploader: FunctionComponent<React.PropsWithChildren<IconImageUploaderProps>> = ({
  ...
  const handleRejectedImage = useCallback(
    (rejection: FileRejections) => {
      ...
      if (rejection.errors.some((error) => error === FileRejectStatus.FileWrongType)) {
        innerErrorMessages.push(translate('Message.WrongFormat'));
      }
      if (rejection.errors.some((error) => error === FileRejectStatus.FileTooBig)) {
