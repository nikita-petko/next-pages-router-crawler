...
import type { RobloxGameInternationalizationApiGetNameDescriptionHistoryV2RequestContentTypeEnum as TranslationProductType } from '@rbx/client-gameinternationalization/v1';
...
import type { GameStringTranslationInfo, HistoryEntry, TranslationInfo } from '../types';
    ...
    async (response: TranslationHistoryResponse | GameContentTranslationHistory) => {
      ...
      const innerTranslationHistory = response.map((history) => {
        if (typeof history.translator?.id === 'undefined') {
          throw new TypeError('Translator Id undefined');
        }
        const translatorType = history.translator?.agentType;
        let translatorName = '';
        if (translatorType === ChangeAgentType.Automation) {
