 ...
 */
function setPassRegionalPricing(
...
export function updateBatchGetGamePassesQueryData(params: {
  ...
  return { ...oldData, gamePasses: updatedPasses as GamePassConfig[] };
}
...
export function useBulkUpdateRegionalPricingForPasses(
  ...
  return useMutation({
    ...
    onSuccess: (response, variables, onSuccessResult, context) => {
      ...
      queryClient.invalidateQueries({ queryKey: gamePassKeys.all(universeId) });
