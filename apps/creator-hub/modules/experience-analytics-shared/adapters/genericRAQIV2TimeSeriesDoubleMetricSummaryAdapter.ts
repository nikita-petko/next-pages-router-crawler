...
import { isRAQIV2DoubleMetricSummaryType } from '../enums/RAQIV2SummaryType';
    ...
    case ChartSummaryType.QuotaPercentageUsage: {
      ...
      const { count, sum } = primarySeries.dataPoints.reduce(
        (acc, [primaryTimestamp, primaryValue]) => {
          ...
          if (primaryValue !== null && secondaryValue != null && secondaryValue !== 0) {
            return {
              count: acc.count + 1,
...
const genericRAQIV2TimeSeriesDoubleMetricSummaryAdapter = ({
  ...
  if (!primarySeries || !secondarySeries) {
    logAnalyticsError(
      // eslint-disable-next-line @typescript-eslint/no-base-to-string, @typescript-eslint/restrict-template-expressions -- ADHOC-cleanup - pre-existing tech debt surfaced by PR #13823 (proxy-module cleanup)
