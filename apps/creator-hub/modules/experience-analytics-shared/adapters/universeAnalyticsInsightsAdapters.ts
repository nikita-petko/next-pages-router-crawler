...
import type { Chart, Insight, Recommendation } from '@modules/clients/analytics';
import { RecommendationType } from '@modules/clients/analytics';
import type { Asset } from '@modules/clients/assetsupload';
...
const adaptChart = (
  ...
  if (
    ...
    throw new Error(
      `GetInsightsResponse - Invalid chart chartKey=${chart?.key ?? 'missing'} isKnownChartKey=${
        chart?.key ? isCentralizedPredefinedChartKey(chart.key) : false
      }`,
    );
  }
  const { startUtcTime, endUtcTime, ...givenOtherContext } = chart.context;
  // TODO: Consider not relying on backend to provide the correct context shape.
