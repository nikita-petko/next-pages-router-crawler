...
const sliceRAQIV2QueryResultByTimeRange = (
  ...
  return {
    values: result.values.map((metricValue) => ({
      // Clone rows so slicing one response into multiple windows doesn't share
      // breakdown-row references across slices.
      breakdownValue: metricValue.breakdownValue?.map((value) => ({ ...value })),
