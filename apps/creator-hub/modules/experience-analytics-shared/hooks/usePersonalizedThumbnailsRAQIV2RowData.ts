...
import type { TPersonalizedThumbnailsTableColumnKey } from '../constants/PersonalizedThumbnailsTableConfig';
import {
...
const usePersonalizedThumbnailsRAQIV2RowData = (
    ...
    (results: UseQueryResult<RAQIV2QueryResponses>[]) => {
      ...
      const isForbidden =
        ...
        results.some((result) => {
          ...
          return resErr && resErr.status === HttpStatusCodes.FORBIDDEN.valueOf();
        });
  ...
  return useMemo(() => {
    const { rowData, isDataLoading, isResponseFailed, isUserForbidden } = results;
    return {
      rowData,
      isDataLoading,
      isResponseFailed,
