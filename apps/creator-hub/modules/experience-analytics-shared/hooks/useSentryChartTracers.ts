...
import { sentryChartTracingEnabled as sentryChartTracingEnabledFlag } from '@generated/flags/creatorAnalytics';
...
const getOptionalPresetKey = (
  ...
  switch (componentKeyOrConfig.type) {
    ...
    default: {
      ...
      throw new Error(`Unknown component type: ${String(exhaustiveCheck)}`);
    }
  }
};
...
const useSentryChartTracers = ({
  ...
  const { ready: sentryChartTracingReady, value: sentryChartTracingEnabledValue } = useFlag(
    sentryChartTracingEnabledFlag,
  );
  const sentryChartTracingEnabled = sentryChartTracingReady && sentryChartTracingEnabledValue;
