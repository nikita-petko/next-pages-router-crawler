...
import usePaginatedRequest, { PaginationResponse } from '../../../hooks/usePaginatedRequest';
import useMappedApiRequest from '../../../hooks/useMappedApiRequest';
import { TablePaginationSpec } from './types';

export type TableDataColumnConfig<TColumnKey> = Omit<TableColumnConfig<TColumnKey>, 'sort'> & {
  sort?: { direction: TableSortOrder; isServerSideSorting?: boolean; isFixedOrder?: boolean };
};
...
const GenericDataTable = <TRowData, TRowKey extends string | number, TColumnKey extends string>({
    ...
    if (tableConfig?.defaultActiveSort && !columnConfig) {
      ...
      throw new Error(
        `Invalid defaultActiveSort: "${tableConfig.defaultActiveSort}" not found in column keys: [${availableColumnKeys}]. ` +
          `Make sure defaultActiveSort is a column key (like RAQIV2PredefinedTableColumnKey.FunnelUsersRealtime), not a dimension (like RAQIV2Dimension.FunnelStep).`,
      );
    }
  ...
  }, [tableConfig?.defaultActiveSort, columnKeys, columnConfigs]);
  ...
  // Build table data from combined row data
  const tableData = useMemo(() => {
    return effectivePrimaryData
      .map(({ rowId }): Map<TColumnKey, CellDataType> => {
        const existingRowData = allRowData.get(rowId);
        if (existingRowData && existingRowData.size === columnKeys.length) {
          return existingRowData;
        }
        return new Map<TColumnKey, CellDataType>();
      })
      .filter((row) => row.size > 0);
  }, [effectivePrimaryData, allRowData, columnKeys]);
  ...
  const paginationSpec = useMemo((): GenericTablePaginationSpec | null => {
    if (!pagination?.initialPageSize) return null;
  ...
  const isDataLoading = isPrimaryLoading || isOtherPhaseLoading;
