...
import type { ExperienceAnalyticsCurrentFilterBundle } from '../AnalyticsCurrentFilterBundleProvider';
import { AnalyticsCurrentFilterBundleContext } from '../AnalyticsCurrentFilterBundleProvider';
...
export const PageConfigAwareFilterProvider: FC<PageConfigAwareFilterProviderProps> = ({
  ...
  const onUnsupportedDimensionFilterDelete = useCallback(
    (dimension: TRAQIV2Dimension) => {
      rawParams.clearFilterDimensions([dimension]);
    },
    [rawParams],
  );
  ...
  const clearFilterDimensions = useCallback(
    (dimensions: ReadonlyArray<TRAQIV2Dimension>) => {
      rawParams.clearFilterDimensions(dimensions);
    },
    [rawParams],
  );
  ...
  const rawBundle = useMemo(
    ...
    [knownFilters, onKnownFiltersChange, onUnsupportedDimensionFilterDelete, clearFilterDimensions],
  );
