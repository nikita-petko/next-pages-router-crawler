...
import useApiRequest, { TUseApiRequestOptions } from '../../hooks/useApiRequest';
...
const getAnalyticsApiDataProvider = <T,>() => {
  const AnalyticsApiDataContext = React.createContext<AnalyticsApiDataState<T>>({
    data: null,
    isLoading: false,
  ...
  const useAnalyticsApiData = (): AnalyticsApiDataState<T> => {
    ...
    if (data === null) {
      throw new Error('useAnalyticsApiData must be used within a AnalyticsApiDataContext');
    }
    return data;
  };
