...
import {
  onboardingTipsConfigs,
  OnboardingFeatureKey,
  OnboardingStepKey,
  featureKeyToStepKeyOrder,
  getStepIndex,
} from '../constants/onboardingTipsConfigs';
import OnboardingTipsCarouselContent from '../components/OnboardingTips/OnboardingTipsCarouselContent';
...
export type TOnboardingTipsContext = {
  ...
  getCurrentOnboardingTipsItem: (onboardingStep: OnboardingStepKey) => ReactNode;
...
export const OnboardingTipsContext = createContext<TOnboardingTipsContext>({
  ...
  getCurrentOnboardingTipsItem: unimplementedFn,
...
type OnboardingFeatureTipsBundle = {
  ...
  setPrevStep: () => void;
  setNextStep: () => void;
  currentOnboardingTipsItem: ReactNode;
  registerComponentRefForScrolling: (
    step: OnboardingStepKey,
    componentRef: RefObject<HTMLSpanElement | null>,
  ) => void;
...
export const useOnboardingTipsStepBundle = (
  ...
  return useMemo(() => {
    ...
    return {
      ...
      currentOnboardingTipsItem: context.getCurrentOnboardingTipsItem(currentFeatureStep),
...
export const OnboardingTipsProvider: FunctionComponent<React.PropsWithChildren> = ({
  children,
}) => {
  ...
  const getCurrentOnboardingStep = useCallback(
    (key: OnboardingFeatureKey) => {
      ...
      return (
        featureCurrentStepRecord.get(key) ||
        (steps && steps[0]) ||
  ...
  const getCurrentOnboardingTipsItem = useCallback((onboardingStep: OnboardingStepKey) => {
    const { titleKey, contentKey, contentLink } = onboardingTipsConfigs[onboardingStep];
    return (
