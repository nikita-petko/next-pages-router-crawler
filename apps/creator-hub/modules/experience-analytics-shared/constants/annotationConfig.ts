...
import { hourInMilliseconds } from '@modules/charts-generic/utils/dateUtils';
...
const AnnotationConfigInternal = {
  ...
  [AnnotationType.Benchmark]: {
    ...
    perMetricConfigs: {
      [RAQIV2Metric.ForwardD1Retention]: {
        type: MetricAnnotationType.DateRangeShifted,
        shiftValue: -1 * hourInMilliseconds * 24, // 1 day
      },
      ...
      [RAQIV2Metric.ForwardD7Retention]: {
        type: MetricAnnotationType.DateRangeShifted,
        shiftValue: -7 * hourInMilliseconds * 24, // 7 days
      },
      ...
      [RAQIV2Metric.ForwardD30Retention]: {
        type: MetricAnnotationType.DateRangeShifted,
        shiftValue: -30 * hourInMilliseconds * 24, // 30 days
      },
    },
    showForUnconfiguredMetrics: true,
  },
  [AnnotationType.CreatorRegexChange]: {
    labelKey: translationKey('Label.Annotation.CreatorRegexChange', TranslationNamespace.Analytics),
    category: AnnotationSelectableCategory.Insights,
    perMetricConfigs: {},
    showForUnconfiguredMetrics: true,
  },
        ...
        sourceMetricContextInPriorityOrder: [
          {
            ...
            descriptionLink: creatorHub.docs.getCloudServicesMemoryStoresMemoryUsageUrl(),
          },
          {
  ...
  [AnnotationType.RetentionCorhortDisclaimer]: {
    ...
    // Not user-selectable: this is a data-validity disclaimer that must appear
    // whenever a forward-looking retention metric is charted, regardless of the
    // annotations dropdown selection or URL params. Visibility is driven purely
    // by the per-metric targeting below (showForUnconfiguredMetrics: false +
    // perMetricConfigs), mirroring how Announcement targets via the API.
  ...
  [AnnotationType.Announcement]: {
    ...
    category: 'NotSelectableCategory',
    perMetricConfigs: {},
    // Target filtering done upstream in useTimeSeriesAnnotations.getCurrentSupportedAnnotations
    // via API-driven metric and dimension targeting instead of FE perMetricConfigs,
    // so we don't need to filter again at the visualization layer
    showForUnconfiguredMetrics: true,
  },
  [AnnotationType.ExtendedServicesEnablement]: {
    labelKey: translationKey(
      'Label.Annotation.ExtendedServicesEnablement',
      TranslationNamespace.Analytics,
    ),
    category: null,
    // Render only on charts whose metric is CpuCoreUtilization (currently the
    // single chart that visualizes the rcc / ccu-cores quota series). The
    // entry value is a `DateRangeShifted` no-op (shiftValue 0) — its sole
    // purpose is to make `perMetricConfigs[metric]` truthy for this metric
    // in `getCurrentSupportedAnnotations` + `useTimeSeriesWebbloxAnnotations`,
    // without applying any per-metric tweak to the rendered marker.
    perMetricConfigs: {
  ...
  [AnnotationType.ConfiguredAlertIncident]: {
    labelKey: translationKey(
      'Label.Annotation.ConfiguredAlertIncident',
      TranslationNamespace.Analytics,
    ),
    // `null` category makes it appear as its own option in the dropdown so users can turn it off.
    category: null,
    // Per-chart visibility is decided dynamically in
    // useTimeSeriesAnnotations.getCurrentSupportedAnnotations by matching the
    // incident's resolved UI metric against the chart's metric, so we keep
    // perMetricConfigs empty and showForUnconfiguredMetrics true.
    perMetricConfigs: {},
    showForUnconfiguredMetrics: true,
  },
