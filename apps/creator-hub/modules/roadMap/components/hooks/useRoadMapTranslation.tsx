...
import type { RoadMapSubheadingData, RoadMapItemData, RoadMapTranslationsDatum } from './types';
...
export default function useRoadMapTranslation() {
  ...
  const roadMapDetails = useMemo(() => {
    return Object.keys(roadmapDetails).map((headingTranslKey: string) => {
      const record = roadmapDetails[headingTranslKey];
      return {
        title: translate(headingTranslKey),
