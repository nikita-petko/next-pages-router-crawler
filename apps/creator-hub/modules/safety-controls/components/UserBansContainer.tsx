...
const { EmptyState, Flex } = components;
const UserBansContainer: FunctionComponent<React.PropsWithChildren<unknown>> = () => {
  const {
  ...
  const addUsersToBanUrl = urls.creatorHub.dashboard.getAddUsersToBanUrl(experienceId);
  ...
  useEffect(() => {
    if (areTranslationsReady) {
      fetchNextPageUserRestrictionsData(pageSize, '', 0);
    }
