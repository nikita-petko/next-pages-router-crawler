...
interface UpdateCampaignConfirmDialogProps extends BaseInjectedDialogProps {
  onConfirm: () => void | Promise<void>;
}

const UpdateCampaignConfirmDialog = ({
  onClose,
  onConfirm,
  setDismissible,
}: UpdateCampaignConfirmDialogProps): ReactElement => {
  ...
  return (
    <BaseDialog
      dialogDescription={translateCampaign('Description.ChangesEffectTime')}
      dialogFooter={
        <>
          <Button
            data-testid='modalPublish'
            isDisabled={isPending}
            isLoading={isPending}
            onClick={handleConfirm}
            size='Medium'
            variant='Emphasis'>
            {translateCampaign('Action.ConfirmUpdate')}
          </Button>
...
export const openUpdateCampaignConfirmDialog = (onConfirm: () => void | Promise<void>): void => {
  openDialog({
    component: UpdateCampaignConfirmDialog,
    props: { onConfirm },
  });
};
