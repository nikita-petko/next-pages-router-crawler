...
import type { TProduct, TTag } from './types';
...
export default class UnifiedLogger {
  ...
  constructor({
    ...
    this.sessionService = this.disableSession
      ? { getOrCreateSessionId: () => {} } // dummy session service if session is disabled
      : new SessionService({
  ...
  logPageLoadEvent() {
    ...
    if (currentUrl === this.lastPageLoadUrl) {
      return;
    }
