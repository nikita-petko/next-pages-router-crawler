...
const useConversationReducer = ({
  ...
  const onStartStreamingResponse = useCallback(
    (
      ...
      if (!matchesActiveRequest(requestId)) return;
  ...
  const onFinishStreamingResponse = useCallback(
    (requestId: RequestId) => {
      if (!matchesActiveRequest(requestId)) return;
      if (!state.activeResponseMessage) throw new Error(`Missing state activeResponseMessage`);
  ...
  const onErrorStreamingResponse = useCallback(
    (conversationError: ConversationError, messageId: MessageId) => {
      if (!matchesActiveRequest(messageId)) return;
  ...
  const stopResponse = useCallback(
    (textareaInput: string) => () => {
      if (!state.streaming) throw new Error(`Cannot call Stop if not streaming`);
    ...
    ({
      ...
      (async () => {
        try {
          ...
          if (state.activePromptMessage) messageHistory.push(state.activePromptMessage);
          if (state.activeResponseMessage) messageHistory.push(state.activeResponseMessage);
