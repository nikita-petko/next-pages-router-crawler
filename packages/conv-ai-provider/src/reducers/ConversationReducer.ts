...
import { MessageRole, ConnectionState, MessageRequestType } from '../types';
...
const createActiveResponseMessage = (
  ...
  if (!state.activePromptMessage) {
    throw new Error(`state.activePromptMessage is required`);
  }
  return {
    messageId,
    createTime: new Date().toISOString(),
  ...
  switch (type) {
    ...
    case ConversationActionType.finishStreamingResponse: {
      if (!state.activeResponseMessage) {
        throw new Error(`state.activeResponseMessage required`);
      }
      return {
        ...state,
        streaming: false,
    ...
    case ConversationActionType.activeMessageConversationError: {
      ...
      // easter egg count
      if (action.error === 'UserRateLimitExceeded') {
        userRateLimitErrorCountNew += 1;
      }
