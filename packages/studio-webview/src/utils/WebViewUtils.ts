...
type CSSStyleRule = Record<string, CSSStyleValue>;
export type WebViewLoadingStyleMap = Record<string, CSSStyleRule>;
export interface WebViewLoadingStyleOptions {
  lightColor?: string;
  darkColor?: string;
}

export function getWebViewLoadingStyles({
  lightColor,
  darkColor,
}: WebViewLoadingStyleOptions = {}): WebViewLoadingStyleMap {
  return {
    '.studio-webview &': {
      height: '100vh',
      position: 'fixed',
