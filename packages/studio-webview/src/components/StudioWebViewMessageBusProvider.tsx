...
import { StudioWebViewMessageBusEventTypesWithDefaults } from '../types/StudioWebViewMessageBusEventTypes';
import {
  RemoveListenerFunc,
  SetListenerFunc,
...
if (
  ...
  (window as any).requestAnimationFrame = (callback: (timestamp: DOMHighResTimeStamp) => void) => {
...
if (
  typeof window !== 'undefined' &&
  !window.DISABLE_STUDIO_WEBVIEW_PREINIT &&
  window.rbx?.postMessage
) {
  const signalReady = () => {
...
const makeStudioWebViewMessageBusProvider = <
  TEventTypes extends StudioWebViewMessageBusEventTypesWithDefaults & {
    [K in keyof TEventTypes]: RequestResponseParams<any, any>;
  },
  TMockMessageBus extends BaseMessageBus<TEventTypes>,
>({
  context,
  bus,
  useSearchParams,
  ...
  MockMessageBus: new () => TMockMessageBus;
}) => {
  return function StudioWebViewMessageBusProvider({
    ...
    const mockMessageBus = useRef<TMockMessageBus | null>(null);
    ...
    useEffect(() => {
      ...
      if (process.env.NEXT_PUBLIC_MOCK_WEB_VIEW === 'true' || mockWebViewSearchParam === 'true') {
        ...
        if (mockThemeOverride) {
          let studioTheme: StudioTheme;
          if (mockThemeOverride.toLowerCase().includes('foundation')) {
            studioTheme = mockThemeOverride.includes('light')
              ? StudioTheme.LightFoundation
              : StudioTheme.DarkFoundation;
          } else {
            studioTheme = mockThemeOverride.includes('light')
              ? StudioTheme.LightStudio
              : StudioTheme.DarkStudio;
          }
    ...
    useEffect(() => {
      // Reset temporary styles set for WebView during load
    ...
    const isFoundationTheme = useMemo(() => {
      return (
        isWebViewAvailable &&
        (currentStudioTheme === StudioTheme.LightFoundation ||
          currentStudioTheme === StudioTheme.DarkFoundation)
      );
    }, [currentStudioTheme, isWebViewAvailable]);
...
>({
  ...
  const provider = makeStudioWebViewMessageBusProvider<TEventTypes, TMockMessageBus>({
    bus,
    context,
    MockMessageBus,
    useSearchParams,
