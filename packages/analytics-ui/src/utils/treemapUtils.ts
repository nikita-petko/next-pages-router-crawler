import type { SingleTreemapSeries, TreemapPoint } from '../types/TreemapChart';
...
export const pruneRedundantOtherNodes = (chartData: TreemapPoint[]): TreemapPoint[] => {
  ...
  chartData.forEach((p) => {
    if (p.parent !== undefined) {
      if (!childrenByParent.has(p.parent)) {
        childrenByParent.set(p.parent, []);
      }
...
export const collapseNodesBelowPercentage = (
  ...
  if (chartData.length === 0 || minDisplayPercentage <= 0) {
    return chartData;
  }
