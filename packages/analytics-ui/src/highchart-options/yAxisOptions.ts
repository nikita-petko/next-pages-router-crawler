...
const useDefaultYAxisOptions = ({
  ...
  YAxisOptions,
  'title' | 'labels' | 'gridLineColor' | 'gridLineDashStyle' | 'gridLineWidth' | 'visible'
> => {
  ...
  return useMemo(() => {
    ...
    switch (chartStyleMode) {
      ...
      default: {
        ...
        throw new Error(`Unhandled chartStyleMode: ${String(exhaustiveCheck)}`);
      }
    }
    ...
    */
      ...
      gridLineColor: getChartThemedColors(theme).gridLine,
  ...
  const getYAxisOptionsByConfig = useCallback(
    (yAxisConfig: YAxisConfig): YAxisOptions => {
      ...
      function wrappedYAxisFormatter(this: AxisLabelsFormatterContextObject): string {
        const { value } = this;
        return yAxisFormatter ? yAxisFormatter({ value }) : String(value);
      }

      function wrappedDefaultFormatterWithDecimalPrecision(
      ...
      return {
        ...
        labels: {
          ...
          formatter: yAxisFormatter
            ? wrappedYAxisFormatter
            : wrappedDefaultFormatterWithDecimalPrecision,
        },
