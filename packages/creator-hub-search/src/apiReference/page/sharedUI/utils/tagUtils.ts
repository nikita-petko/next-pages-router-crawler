...
import type { EngineTag, ReferenceClass } from '../../../../ui/tags/types';
import { ThreadSafetyTag, ApiTag, SecurityTag, CloudTag } from '../../../../ui/tags/types';
...
export const formatTagName = (tag: string): string => tag.replaceAll(/_/g, ' ');
...
const getReadWriteSecurityTags = (member: APIReferenceMemberWithReadWriteSecurityObj): string[] => {
  ...
  if (member.security.write) {
    result.push(member.security.write);
  }
  if (member.security.read) {
    result.push(member.security.read);
  }
  return result;
};
