import type { Dispatch, FunctionComponent, ReactElement, SetStateAction } from 'react';
import React, { useCallback } from 'react';
import { useRobloxAuthentication } from '@rbx/auth';
import type { TMenuItemProps } from '@rbx/foundation-ui';
import { Menu, MenuLabel, MenuSection, MenuSeparator } from '@rbx/foundation-ui';
import { useTranslation } from '@rbx/intl';
import type { AccountSwitcherSnackbarConfig } from '../../../accountSwitcher';
...
type TAuthenticationStatusMenuProps = {
  ...
  isAccountSwitcherFrameLoaded: boolean; // whether the account switcher iframe has loaded. if false, the menu item is still clickable but the dialog shows a loading state. if true, we show the menu item based on whether the account switcher is enabled.
  ...
  return (
    <Menu size='Medium' className='min-width-[260px]'>
      ...
      <MenuSection>
        ...
        />
        {isAccountSwitcherSupported &&
        (!isAccountSwitcherFrameLoaded || isAccountSwitcherEnabled) ? ( // if frame is loaded, show only if enabled. otherwise show clickable menu item while loading (dialog handles the loading state).
          <AuthenticationStatusMenuItem
            key='SwitchAccounts'
