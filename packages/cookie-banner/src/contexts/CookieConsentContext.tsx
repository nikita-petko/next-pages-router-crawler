...
import type { CookiePolicyResponse, EssentialCookie, NonEssentialCookieNameType } from '../types';
import getBaseDomain from '../utils/getBaseDomain';

const COOKIE_CONSENT_COOKIE_NAME = 'RBXcb';
const COOKIE_CONSENT_EXPIRY_DAYS = 180;
const COOKIE_CONSENT_API_ENDPOINT = '/guac-v2/v1/bundles/cookie-policy';

const COOKIE_CONSENT_DEFAULT_PREFERENCES: Record<NonEssentialCookieNameType, boolean> = {
...
const loadPreferencesFromCookie = (): CookiePreferences => {
  ...
  const preferencesString = savedPreferences.substring(COOKIE_CONSENT_COOKIE_NAME.length + 1); // get the value of the cookie
...
export const CookieConsentProvider: React.FC<CookieConsentProviderProps> = ({
  ...
  useEffect(() => {
    ...
    const fetchCookiePolicy = async () => {
      try {
        ...
        setCookiePolicy({
          ShouldDisplayCookieBannerV3: data.ShouldDisplayCookieBannerV3 ?? false,
  ...
  const acceptAll = useCallback(() => {
    ...
    const allAccepted = cookiePolicy.NonEssentialCookieList.reduce<CookiePreferences>(
      (acc, cookie) => ({
        ...acc,
        [cookie]: true,
      }),
  ...
  const declineAll = useCallback(() => {
    ...
    const allDeclined = cookiePolicy.NonEssentialCookieList.reduce<CookiePreferences>(
      (acc, cookie) => ({
        ...acc,
        [cookie]: false,
      }),
