...
const IneligibleLearnMoreLink = ({ children }: { children: React.ReactNode }) => (
  <a
...
const YourPrivacyChoicesModal = ({
  ...
  const adsPreferencesUrl = useMemo(() => getAdsPreferencesUrl(intl.getRobloxLocale()), [intl]);
  ...
  const ineligibleLearnMoreLinkContent = useCallback(
    (chunks: React.ReactNode) => <IneligibleLearnMoreLink>{chunks}</IneligibleLearnMoreLink>,
    [],
  );
  ...
  const getModalDescription = (): React.ReactElement => {
    if (isLoading) {
      ...
      return <div className='text-body-medium'>{translate(constants.descriptionLoading)}</div>;
    }

    if (isError || !data) {
      return (
        <div className='text-body-medium'>{translateHTML(constants.bodyErrorTranslationKey)}</div>
      );
    }

    const { scenario, isAdsSellShareDataEnabled, canUserManageAdsSettings } = data;
      ...
      if (scenario === 'no-signal') {
        return (
          <div className='text-body-medium'>
            {translateHTML(constants.bodyGpcMissingSettingIneligible, config.tags, config.args)}
          </div>
        );
      }
      return (
