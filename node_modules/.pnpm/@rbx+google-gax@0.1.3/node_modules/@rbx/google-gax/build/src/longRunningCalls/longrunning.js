 ...
 *
 * This file was derived and modified from gax/src/longRunningCalls/longrunning.ts
...
const openCloudError_1 = require("../openCloudError");
...
class Operation extends events_1.EventEmitter {
     ...
     * @constructor
     *
     * @param {roblox.longrunning.Operation} grpcOp - The operation to be wrapped.
    ...
    constructor(grpcOp, longrunningDescriptor, backoffSettings, callOptions) {
        ...
        this.path = this.latestResponse.path;
    ...
    cancel() {
        ...
        const cancelRequest = new operationProtos.roblox.longrunning.CancelOperationRequest();
        cancelRequest.path = this.latestResponse.path;
    ...
    getOperation(callback) {
            ...
            if (!callback) {
                return new Promise((resolve, reject) => {
                    if (self.latestResponse.error) {
                        const error = new openCloudError_1.OpenCloudError(self.latestResponse.error.message);
        ...
        const request = new operationProtos.roblox.longrunning.GetOperationRequest();
        request.path = this.latestResponse.path;
    ...
    _unpackResponse(op, callback) {
        ...
        if (op.done) {
            if (op.result === 'error') {
                const error = new openCloudError_1.OpenCloudError(op.error.message);
    ...
    startPolling_() {
        ...
        function retry() {
            ...
            if (now.getTime() >= deadline) {
                const error = new openCloudError_1.OpenCloudError('Total timeout exceeded before any response was received');
                error.code = status_1.Status.DEADLINE_EXCEEDED;
                setTimeout(emit, 0, 'error', error);
                return;
            }
            self.getOperation((err, result, metadata, rawResponse) => {
                if (err) {
                    setTimeout(emit, 0, 'error', err);
                    return;
                }
                if (!result) {
                    if (rawResponse.metadata &&
                        ...
                        setTimeout(emit, 0, 'progress', metadata, rawResponse);
                    ...
                    if (rawResponse.done) {
                        setTimeout(emit, 0, 'complete', {}, metadata, rawResponse);
                        return;
                    }
                    setTimeout(() => {
                        now = new Date();
                ...
                setTimeout(emit, 0, 'complete', result, metadata, rawResponse);
            });
        }
        retry();
    }
 ...
 * @constructor
 *
 * @param {roblox.longrunning.Operation} op - The operation to be wrapped.
 * @param {LongRunningDescriptor} longrunningDescriptor - This defines the
 * operations service client and unpacking mechanisms for the operation.
 * @param {BackoffSettings} backoffSettings - The backoff settings used in
 * in polling the operation.
