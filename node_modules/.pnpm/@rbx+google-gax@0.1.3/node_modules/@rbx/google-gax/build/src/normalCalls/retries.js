 ...
 *
 * This file was derived and modified from gax/src/normalCalls/retries.ts
    ...
    return (argument, callback) => {
        ...
        function repeat() {
            ...
            if (deadline && now.getTime() >= deadline) {
                const error = new openCloudError_1.OpenCloudError(`Total timeout of API ${apiName} exceeded ${retry.backoffSettings.totalTimeoutMillis} milliseconds before any response was received.`);
                error.code = status_1.Status.DEADLINE_EXCEEDED;
                callback(error);
                return;
            }
            if (retries && retries >= maxRetries) {
                const error = new openCloudError_1.OpenCloudError('Exceeded maximum number of retries before any ' +
            ...
            if (canceller instanceof Promise) {
                canceller.catch(err => {
                    callback(new openCloudError_1.OpenCloudError(err));
                });
            }
        }
        if (maxRetries && deadline) {
            const error = new openCloudError_1.OpenCloudError('Cannot set both totalTimeoutMillis and maxRetries ' +
                'in backoffSettings.');
        ...
        return {
            cancel() {
                ...
                if (canceller) {
                    ...
                    const error = new openCloudError_1.OpenCloudError('cancelled');
