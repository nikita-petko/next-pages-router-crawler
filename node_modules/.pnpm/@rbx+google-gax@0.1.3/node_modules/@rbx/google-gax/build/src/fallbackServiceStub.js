 ...
 *
 * This file was derived and modified from gax/src/fallbackServiceStub.ts
 * in github.com/googleapis/gax-nodejss.
 */
...
function generateServiceStub(rpcs, protocol, servicePath, servicePort, authClient, requestEncoder, responseDecoder, numericEnums) {
    ...
    for (const [rpcName, rpc] of Object.entries(rpcs)) {
        serviceStub[rpcName] = (request, options, _metadata, callback) => {
            ...
            // Helper method containing core logic to construct and send HTTP Request
            function constructAndSendFetchRequest() {
                return authClient
                    .getRequestHeaders()
                    .then(authHeader => {
                    const fetchRequest = {
                        ...
                        signal: cancelSignal,
                        // Ensures Cookies are added to every request
                        credentials: 'include'
                    };
            ...
            constructAndSendFetchRequest()
                .then((response) => {
                // Used expired CSRF Token
                if (response.status == 403 && response.headers.has("x-csrf-token")) {
                    // Retry request with new CSRF Token
                    authClient.csrfToken = response.headers.get("x-csrf-token");
                    return constructAndSendFetchRequest();
                }
                else {
                    return response;
                }
                ...
                if (response.ok && rpc.responseStream) {
                    ...
                    return Promise.all([
                        ...
                        .then(([ok, buffer, status]) => {
                        const response = responseDecoder(rpc, ok, buffer, status);
