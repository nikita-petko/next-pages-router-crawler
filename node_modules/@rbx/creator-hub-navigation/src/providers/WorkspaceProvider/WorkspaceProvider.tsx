...
export const WorkspaceProvider: FunctionComponent<React.PropsWithChildren> = ({ children }) => {
  ...
  const setCurrentWorkspace = useCallback(
    (workspace: TWorkspace) => {
      ...
      if (
        !isAcceptedWorkspacePath(pathname, router.query.tab) &&
        currentProduct === ProductKey.CreatorDashboard
      ) {
        void Router.push({
          pathname: creations,
        });
    ...
    [creations, pathname, router.query.tab, currentProduct, setCurrentGroupId, setWorkspaceData],
  );
