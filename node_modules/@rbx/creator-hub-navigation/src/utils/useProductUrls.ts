...
const useProductUrls = () => {
  const { target, robloxEnvironment: environment, currentProduct } = useNavigationConfigs();
  ...
  return useMemo(() => {
    ...
    const adsBasePath = currentProduct === 'Advertise' ? '/' : `${baseUrl}advertise`;
  ...
  }, [currentProduct, target, environment, pathname]);
};
