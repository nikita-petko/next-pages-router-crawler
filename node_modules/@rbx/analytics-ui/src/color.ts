...
/**
 * Extended categorical palette for charts that need many distinguishable
 * series in one view (e.g. dense Sankey stages). Colors are assigned uniquely
 * per node, advancing through the palette across stages without reusing earlier
 * stage colors.
 */
export const ExtendedCategoricalChartColors = [
  ...OrderedChartColors,
  ChartColor.Yellow3,
  ChartColor.Green3,
  ChartColor.Cyan2,
...
/**
 * Dark-theme hex palette retained for internal consumers that need raw CSS
 * values instead of semantic {@link ChartColor} identifiers.
 */
export const ExtendedCategoricalColorsDark: readonly string[] = ExtendedCategoricalChartColors.map(
  (color) => DarkColorHexByChartColor[color],
);
...
export const getChartThemedColors = (theme: TTheme): TChartThemedColors => {
  ...
  return {
    ...
    gridLine: theme.palette.surface.outline,
