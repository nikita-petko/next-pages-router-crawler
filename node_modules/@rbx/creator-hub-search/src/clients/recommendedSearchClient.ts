...
const joinUrlPath = (...segments: string[]): string =>
  segments
    ...
    .map((segment, index) =>
      index === 0 ? segment.replace(/\/+$/, '') : segment.replaceAll(/^\/+|\/+$/g, ''),
    )
    .join('/');
...
const getRrfScore = (data: Data) => ('rrf' in data ? data.rrf : 0);

const isRecommendationData = (data: unknown): data is Data => {
  if (typeof data !== 'object' || data == null || !('value' in data)) {
    return false;
  }
  const candidate = data as { value?: unknown; rrf?: unknown };
  return (
    typeof candidate.value === 'string' &&
    (candidate.rrf === undefined || typeof candidate.rrf === 'number')
  );
};

const isScoredSearchData = (data: SearchData): data is SearchData & ScoredRecommendationData =>
  'rrf' in data;

export const processDatasets = (datasets: Data[][]): SearchData[] =>
  datasets
    .flat()
    .sort((a, b) => getRrfScore(b) - getRrfScore(a))
    .map((data: Data) => ({
      ...data,
...
export class RecommendedSearchClient {
  ...
  async fuzzySearchRecommendations({
    ...
    if (!isScoredSearchData(firstCandidate.item)) {
      return [];
    }
      ...
      [
        {
          ...
          rank: 1,
          rrf: firstCandidate.item.rrf + rrf(1),
        },
      ];
    ...
    for (let i = 1; i < candidates.length; i += 1) {
      ...
      if (isScoredSearchData(item)) {
        rrfCandidates.push({
          ...item,
          score,
          rank,
          rrf: item.rrf + rrf(rank),
        });
      }
    }
  ...
  private static async getDataset(locale: Locale): Promise<Data[]> {
    ...
    return Array.isArray(data) ? data.filter(isRecommendationData) : [];
  }
