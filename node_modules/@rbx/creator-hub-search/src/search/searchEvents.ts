...
import type { DocumentationContentType, SearchDisplayCategory } from '../clients/docSiteSearchType';
import { getDisplayCategory } from '../clients/docSiteSearchType';
import {
  ESearchEventName,
  ESearchInteraction,
  ESearchSource,
...
const filterResultsForCategory = (
  ...
  if (isDisplayCategory) {
    return searchResults.filter((r) => getDisplayCategory(r.documentationContentType) === category);
  }
  return searchResults.filter((r) => (r.documentationContentType as string | null) === category);
};
...
export const trackSearchPerformed = ({
  ...
  eventLogger: UnifiedLogger;
  category: DocumentationContentType | SearchDisplayCategory;
  interaction: TPerformedInteraction;
  isRecommended: boolean;
  isSemantic: boolean;
  // Only the V2 (CreatorHubSearch) backend path returns this; omitted otherwise.
  ...
  eventLogger.logClickEvent({
    ...
    parameters: {
      ...
      ...(isQueryUnderstandApplied !== undefined
        ? { isQueryUnderstandApplied: isQueryUnderstandApplied.toString() }
        : {}),
      locale,
      ...
      ...(searchResults.some((r) => r.storeSearchCategory)
        ? {
            storeSearchCategories: Array.from(
              new Set(searchResults.map((r) => r.storeSearchCategory).filter(Boolean)),
            ).join('|'),
          }
        : {}),
      currentProduct,
...
export const trackSearchResultClicked = ({
  ...
  eventLogger.logClickEvent({
    ...
    parameters: {
      ...
      ...(searchResult.isQueryUnderstandApplied !== undefined
        ? { isQueryUnderstandApplied: searchResult.isQueryUnderstandApplied.toString() }
        : {}),
      locale,
      query,
      ...
      ...(searchResult.storeSearchCategory
        ? { storeSearchCategory: searchResult.storeSearchCategory }
        : {}),
      currentProduct,
      searchSessionId,
...
export const trackSearchCategoryClicked = ({
  ...
  const filteredResults = filterResultsForCategory(searchResults, category);
  const filteredCategories = Array.from(
    new Set(filteredResults.map((r) => r.documentationContentType)),
  );
  const displayIndex = SEARCH_DISPLAY_CATEGORIES.findIndex((c) => c.value === category);
...
export const trackSearchCategoryCleared = ({
  ...
  const numResults = filterResultsForCategory(searchResults, category).length;
  eventLogger.logClickEvent({
...
// (2026/06) Store navigation surfaces. The "Search in Store" row and the Store
// category tiles aren't backend search results, so they fire dedicated events
// (not ResultClicked). They share the same click-interaction taxonomy as result
// clicks (plain / keyboard / modifier).
export type TStoreNavClickedInteraction = TClickedInteraction;

export const trackSearchInStoreClicked = ({
  eventLogger,
  interaction,
  locale,
  query,
  ...
  eventLogger.logClickEvent({
    eventName: ESearchEventName.SearchInStoreClicked,
    parameters: {
      interaction,
      locale,
      query,
...
export const trackSearchStoreCategoryTileClicked = ({
  eventLogger,
  interaction,
  locale,
  query,
  ...
  currentProduct: string;
  searchSessionId: string;
  // Tile slug, e.g. '3d' / 'plugins' (distinct from the 'Model'/'Plugin'
  // categoryType that real Store results carry).
  categoryId: string;
  ...
  eventLogger.logClickEvent({
    eventName: ESearchEventName.StoreCategoryTileClicked,
    parameters: {
      interaction,
      locale,
      query,
      ...
      resultUrl: destinationUrl,
      storeSearchCategory: categoryId,
      currentProduct,
      searchSessionId,
      source: ESearchSource.SearchResults,
    },
